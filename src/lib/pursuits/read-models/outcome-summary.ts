...
export async function getPursuitOutcomeSummary(db: PoolClient, orgId: string, pursuitId: string): Promise<PursuitOutcomeSummary> {
  ...
  if (oc?.attribution_id) {
    const at = (await db.query<{ attribution_class: string; human_override_class: string | null; subject_kind: string; subject_label: string | null; reason: string | null; model_version: string }>(
      `select attribution_class, human_override_class, subject_kind, subject_label, reason, model_version from attribution where id = $1 and org_id = $2`, [oc.attribution_id, orgId])).rows[0];
    if (at) attribution = {
  ...
  const recomputePending = hasRecompute
    ? Number((await db.query<{ n: string }>(`select count(*)::text n from recompute_requests where pursuit_id = $1 and org_id = $2 and status in ('PENDING','RUNNING')`, [pursuitId, orgId])).rows[0].n) > 0
    : false;
