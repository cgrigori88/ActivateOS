...
export function PursuitHero({ d, lifecycleWord }: { d: PursuitDetailView; lifecycleWord: string }) {
  ...
  return (
    <div>
      <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.08em] text-neutral-500">
        <span style={{ color: "var(--color-priority)" }}>Pursuit</span>
        <span aria-hidden>·</span>
        <span>{d.accountLabel}</span>
        {d.synthetic && <SyntheticBadge text="demo" />}
      </div>
      <h1 className="mt-1.5 text-[30px] font-extrabold leading-[1.08] tracking-[-0.03em]">{d.thesis}</h1>
      ...
      <div className="mt-3.5 flex flex-wrap gap-x-7 gap-y-2">
        {meta.map(([k, v]) => (
          <div key={k}>
            <div className="text-[10px] font-bold uppercase tracking-[0.05em] text-neutral-400">{k}</div>
...
function WhyNowRow({ c, kind, forceLabel }: { c: WhyNowComponent | null; kind: string; forceLabel: string }) {
  return (
    <div className="grid grid-cols-[150px_1fr] gap-3 py-2.5" style={{ borderTop: "1px solid var(--border-subtle)" }}>
      <div className="flex items-center gap-2 text-[12px] font-semibold text-neutral-600 dark:text-neutral-300">
        <span aria-hidden className="text-neutral-400">{WHY_ICON[kind] ?? "•"}</span>{c?.label ?? forceLabel}
      </div>
      <div>
        {c?.present && c.detail
          ? <>
              <div className="text-[13.5px] font-semibold">{c.detail}</div>
              {c.commercialImplication && <div className="mt-0.5 text-[11.5px] text-neutral-500">{c.commercialImplication}</div>}
              {c.refId && <div className="mt-0.5 text-[11px] font-semibold" style={{ color: "var(--color-band-high)" }}>↳ traceable to source</div>}
            </>
...
export function WhyNowBento({ w }: { w: WhyNowView }) {
  if (!w.present) return <p className="text-[13px] italic text-neutral-500">No structured Why Now assembled yet.</p>;
  return (
    <div>
      ...
      {w.contradictions.length > 0 && (
        <div className="mt-3 rounded-card p-3 text-[12.5px]" style={{ background: "color-mix(in srgb, var(--color-accent-risk) 8%, transparent)" }}>
      ...
      {w.unknowns.length > 0 && (
        <div className="mt-3 rounded-card p-3" style={{ background: "var(--surface-inset)" }}>
          <div className="text-[10px] font-bold uppercase tracking-[0.05em] text-neutral-500">What we don’t know yet</div>
          <ul className="mt-1 space-y-0.5 text-[12px] text-neutral-600 dark:text-neutral-300">
...
export function FactsBento({ facts }: { facts: FactItem[] }) {
  if (!facts.length) return <p className="text-[13px] italic text-neutral-500">No facts surfaced yet.</p>;
  return (
      ...
      {facts.map((f) => {
        ...
        return (
          <div key={f.id} className="flex items-start justify-between gap-3 rounded-card p-3.5" style={{ background: `color-mix(in srgb, ${hue} 5%, var(--surface-primary))`, boxShadow: "var(--shadow-low)" }}>
            <div>
              <div className="text-[13.5px] font-semibold">{f.proposition}</div>
            ...
            {f.confidence && (
              <div className="flex-none text-right">
                ...
                {f.confidence.known && <div className="tnum mt-0.5 text-[10.5px] text-neutral-400">confidence {f.confidence.value}</div>}
              </div>
            )}
          </div>
        );
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        {team.members.map((m, i) => (
          <div key={i} className="flex items-center justify-between gap-2 rounded-card px-3.5 py-2.5" style={{ background: "var(--surface-inset)" }}>
            <div>
              <div className="text-[12.5px] font-semibold">{m.personLabel ?? m.role.replace(/_/g, " ").toLowerCase()}</div>
              <div className="text-[11px] text-neutral-400">{m.personLabel ? m.role.replace(/_/g, " ").toLowerCase() : `${m.side.toLowerCase()} side`}</div>
            </div>
...
export function MaterialChangeTimeline({ timeline }: { timeline: PursuitTimelineView }) {
  if (!timeline.events.length) return <p className="text-[13px] italic text-neutral-500">No material changes yet.</p>;
  return (
    <div>
        ...
        return (
          <div key={i} className="grid grid-cols-[14px_1fr] gap-3 py-2.5" style={{ borderTop: i ? "1px solid var(--border-subtle)" : "none" }}>
            ...
            <div>
              ...
              <div className="tnum mt-0.5 text-[11px] text-neutral-400">{new Date(e.at).toLocaleString()} · {e.changeType.replace(/_/g, " ").toLowerCase()}</div>
            </div>
          </div>
        );
      })}
