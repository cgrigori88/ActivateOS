...
export function FederationBento({ fed, actions, outcomes }: {
  ...
  return (
    <div className="flex flex-col gap-5">
      {/* Participants */}
      <div>
        <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.04em] text-neutral-400">Participants on this pursuit</div>
          ...
          {fed.participants.map((p) => (
            <li key={p.orgId} className="flex items-center justify-between gap-3 rounded-md border border-neutral-200/70 bg-white/40 px-3 py-2 dark:border-neutral-700/60 dark:bg-white/5">
              <span className="flex items-center gap-2 text-[13px]">
                ...
                <span className="text-[11.5px] text-neutral-500">{p.roleKey}</span>
              </span>
      ...
      <div>
        <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.04em] text-neutral-400">Shared context you may see</div>
        {fed.sharedContext.length === 0 ? (
          <p className="text-[12.5px] text-neutral-500">No shared context is disclosed to you on this pursuit.</p>
        ) : (
          <ul className="flex flex-col gap-1.5">
            {fed.sharedContext.map((c, i) => (
              <li key={i} className="flex items-start justify-between gap-3 text-[12.5px]">
                ...
                <span className="shrink-0 rounded-full border border-neutral-200/70 px-1.5 py-0.5 text-[10px] uppercase tracking-[0.03em] text-neutral-400 dark:border-neutral-700/60">{c.visibility}</span>
              </li>
            ))}
          </ul>
        )}
      ...
      <div>
        <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.04em] text-neutral-400">Actions you can take</div>
        <div className="flex flex-wrap gap-1.5">
          {actions.eligible.length === 0
            ? <span className="text-[12.5px] text-neutral-500">No governed actions are available to you here.</span>
            : actions.eligible.map((a) => (
              <span key={a.skillId} className="inline-flex items-center gap-1.5 rounded-md border border-neutral-200/70 bg-white/40 px-2 py-1 text-[12px] dark:border-neutral-700/60 dark:bg-white/5" title={a.description}>
                {a.description}
                <span className="rounded-sm bg-neutral-100 px-1 text-[10px] uppercase tracking-[0.02em] text-neutral-500 dark:bg-white/10">{EFFECT_WORD[a.effectClass] ?? a.effectClass}</span>
              </span>
            ))}
        </div>
      ...
      {outcomes && outcomes.outcomes.length > 0 && (
        <div>
          <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.04em] text-neutral-400">Outcome trail</div>
