      ...
      {funnels.map((f) => {
        ...
        return (
          <div key={f.hypothesis.taxonomyNodeId} className="pos-card rounded-card p-4">
            ...
            <div className="mt-2.5 flex flex-wrap items-center gap-2">
              {(Object.keys(COHORT_META) as (keyof typeof COHORT_META)[]).map((k) => f.cohorts[k as keyof typeof f.cohorts] > 0 && (
                <Link key={k} href={qs({ mdrawer: f.hypothesis.taxonomyNodeId, mstage: k })} scroll={false}
                  className="rounded-full px-2 py-px text-label font-semibold hover:opacity-80"
              ...
              {notReady > 0 && (
                <Link href={qs({ mdrawer: f.hypothesis.taxonomyNodeId, mstage: "not_ready" })} scroll={false}
                  className="ml-auto rounded-control px-2.5 py-1 text-body font-semibold"
            ...
            <p className="mt-2.5 text-label text-neutral-500">
      ...
      {funnels.map((f) => {
        ...
        return (
          <div key={f.hypothesis.taxonomyNodeId} className="pos-card rounded-card p-4">
            ...
            {agg.rows.length === 0 ? (
              <p className="mt-2 text-body text-neutral-500">Every evaluated account is execution-ready.</p>
            ) : (
            ...
            {agg.overlays.length > 0 && (
              <div className="mt-2 border-t border-neutral-200/70 pt-1.5 dark:border-neutral-800">
                <span className="px-2.5 text-micro font-bold uppercase tracking-[0.05em] text-neutral-400">Informational — never gates</span>
            ...
            <p className="mt-2 text-micro text-neutral-400">Grouped by each pursuit&rsquo;s primary blocker — the first failing canonical gate. Click a row for the exact pursuits.</p>
          </div>
        );
      })}
    </section>
    ...
    <section className="mb-6 space-y-3">
      {funnels.map((f) => (
        <div key={f.hypothesis.taxonomyNodeId} className="pos-card rounded-card p-4">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            <h2 className="text-title font-extrabold tracking-[-0.01em]">{f.hypothesis.name}</h2>
            <span className="text-label text-neutral-500">{f.accounts.length} evaluated · ranked by expected value</span>
          </div>
          <div className="mt-2 overflow-x-auto">
            <table className="w-full min-w-[720px] border-collapse text-body">
              <thead>
                <tr className="text-left text-micro font-bold uppercase tracking-[0.04em] text-neutral-500">
                ...
                {f.accounts.slice(0, TABLE_CAP).map((a) => {
                  ...
                  return (
                    <tr key={a.companyId} style={{ borderTop: "1px solid var(--border-subtle)" }}>
                      ...
                      <td className="px-2 py-1.5">
                        <span className="rounded-full px-1.5 py-px text-micro font-semibold" style={{ color: hue, background: `color-mix(in srgb, ${hue} 12%, transparent)` }}>{COHORT_META[a.cohort].label}</span>
                      </td>
                      <td className="px-2 py-1.5">
                        {p ? (
          ...
          {f.accounts.length > TABLE_CAP && (
            <p className="mt-2 text-label text-neutral-400">Showing the top {TABLE_CAP} by expected value — narrow the scope to see a specific cut.</p>
          )}
        </div>
      ))}
    </section>
  ...
  return (
    <div className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-label={title}>
      ...
      <aside className="absolute inset-y-0 right-0 flex w-[min(480px,94vw)] flex-col overflow-y-auto border-l p-4 scroll-thin"
        ...
        {accounts.length === 0 ? (
          <p className="text-copy italic text-neutral-500">No accounts in this cut.</p>
        ) : (
          <div className="space-y-2">
            {accounts.slice(0, DRAWER_PAGE).map((a) => <AccountRow key={a.companyId} a={a} />)}
            {accounts.length > DRAWER_PAGE && (
              <p className="text-label text-neutral-400">+ {accounts.length - DRAWER_PAGE} more — narrow the scope or open the Pursuits view.</p>
            )}
          </div>
        )}
      </aside>
  ...
  return (
    <div className="rounded-card p-3" style={{ background: "var(--surface-inset)", boxShadow: "inset 0 0 0 1px var(--border-subtle)" }}>
      ...
      <div className="flex items-baseline justify-between gap-2">
        <Link href={a.pursuitId ? `/pursuits/${a.pursuitId}` : `/accounts/${a.companyId}`} className="min-w-0 truncate text-copy font-semibold hover:underline">{a.name}</Link>
      ...
      {p ? (
        <div className="mt-1.5">
          <span className="text-micro font-bold uppercase tracking-[0.06em]" style={{ color: severityHue(p.severity) }}>Primary blocker</span>
        ...
        <p className="mt-1 text-body" style={{ color: "var(--color-accent-verified)" }}>All gates pass — execution-ready.</p>
      )}
      ...
      {(rest > 0 || info.length > 0) && (
        <details className="mt-1">
          ...
          <ul className="mt-1.5 space-y-1">
            ...
            {info.map((c, i) => (
              <li key={`i${i}`} className="text-label text-neutral-400">◦ {c.label}{c.remedy && <> · <Link href={c.remedy.deepLink} className="hover:underline">{c.remedy.label}</Link></>}</li>
            ))}
          </ul>
        </details>
      )}
