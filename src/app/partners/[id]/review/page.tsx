...
function Metric({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="rounded-inner border border-neutral-200 px-3 py-2 dark:border-neutral-800 print:border-neutral-300">
      <div className="text-label uppercase tracking-wide text-neutral-400">{label}</div>
      <div className="tnum text-section font-semibold">{value}</div>
  ...
  return (
    <main className="mx-auto max-w-4xl">
      ...
      <div className="mb-4 flex items-center justify-between gap-3 print:hidden">
        <Link href={`/partners/${partner.id}`} className="text-copy text-neutral-500 hover:underline">← {partner.name}</Link>
        <PrintButton />
      </div>
      ...
      <header className="mb-6 border-b-2 border-neutral-900 pb-3 dark:border-neutral-100 print:border-neutral-900">
        ...
        <p className="mt-1 text-body italic text-neutral-400">
      ...
      <Section title="Initiatives" note="target vs what the pipeline actually did">
        {initiatives.length === 0 ? (
          <p className="text-copy text-neutral-500">No initiatives on record with this partner.</p>
        ) : (
            ...
            <tbody>
              {initiatives.map((i) => (
                <tr key={i.id} className="border-t border-neutral-100 dark:border-neutral-800 print:border-neutral-200">
                  <td className="py-1.5 pr-2 font-medium">{i.name}{i.status !== "active" && <span className="ml-1 text-body text-neutral-400">({i.status})</span>}</td>
      ...
      <Section title="Coverage" note="their book as ingested and matched">
        {book.length === 0 ? (
          <p className="text-copy text-neutral-500">No account lists from this partner yet.</p>
        ) : (
          <ul className="grid gap-1 text-copy md:grid-cols-2">
            {book.map((l) => (
              <li key={l.id} className="flex justify-between gap-2 border-b border-neutral-100 py-1 dark:border-neutral-800 print:border-neutral-200">
                <span>{l.name} <span className="text-body text-neutral-400">· {l.category}</span></span>
          ...
          <ul className="text-copy">
            {renewals.map((r) => (
              <li key={r.companyId} className="flex justify-between gap-2 border-b border-neutral-100 py-1 dark:border-neutral-800 print:border-neutral-200">
                <span>{r.legalName}{r.listName && <span className="text-body text-neutral-400"> · on “{r.listName}”</span>}</span>
                <span className="tnum text-neutral-500">{r.label} {r.phrase} <span className="text-body text-neutral-400">· {r.sourceNote}</span></span>
              </li>
            ))}
          </ul>
        </Section>
      ...
      {(playbook.positioning || playbook.strengths || playbook.rules) && (
        <Section title="Playbook" note="internal — never shared with the partner">
          <div className="grid gap-3 text-copy md:grid-cols-3">
            {playbook.positioning && <div><p className="mb-0.5 text-body font-semibold text-neutral-400">Joint positioning</p><p className="text-neutral-600 dark:text-neutral-300">{playbook.positioning}</p></div>}
            {playbook.strengths && <div><p className="mb-0.5 text-body font-semibold text-neutral-400">Their strengths</p><p className="text-neutral-600 dark:text-neutral-300">{playbook.strengths}</p></div>}
            {playbook.rules && <div><p className="mb-0.5 text-body font-semibold text-neutral-400">Rules of engagement</p><p className="text-neutral-600 dark:text-neutral-300">{playbook.rules}</p></div>}
          </div>
        </Section>
      )}
