...
export default async function ContactsPage({
  ...
  const { typed, metaRows, discovered } = await withTenant(async (db, orgId) => {
    ...
    // play at the account (its top-fit product line). Keyed by company, limited to
    // the companies the org's contacts and discovered people actually hang off.
    const metaIds = [...new Set([...typed.map((t) => t.company_id), ...discovered.map((d) => d.company_id)].filter(Boolean))] as string[];
