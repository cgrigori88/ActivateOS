...
export function MappingReview({
  ...
  return (
    <form action={commit} className="space-y-6">
            ...
            {headers.map((h, i) => {
              ...
              return (
                <tr key={i} className={c.target ? "" : "opacity-50"}>
                  ...
                  <td className="text-body text-neutral-500">
                    {p?.type ?? "text"} · {Math.round((p?.fillRate ?? 0) * 100)}% filled
                  </td>
                  <td className="max-w-56 truncate text-body text-neutral-400" title={p?.samples.join(" · ")}>
                  ...
                  <td className="text-center">
                    {c.target && c.target !== "company" ? (
                      ...
                      <span className="text-body text-neutral-300 dark:text-neutral-700">—</span>
                    )}
      ...
      {!companyMapped && (
        <p className="rounded-inner border border-amber-200 bg-amber-50/70 px-3 py-2 text-copy text-amber-800 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-300">
      ...
      {keptCols.length > 0 && preview.length > 0 && (
        <div>
          <h3 className="mb-1.5 text-body font-semibold uppercase tracking-wide text-neutral-500">
            ...
            <table className="data-table">
              ...
              <tbody>
                {preview.map((row, ri) => (
                  <tr key={ri}>
                    {keptCols.map((c) => (
                      <td key={c.i} className="max-w-48 truncate text-body" title={row[c.i]}>
      ...
      {kind === "crm" && (
        <div className="rounded-xl border border-amber-300 bg-amber-50 p-4 text-copy text-amber-800 dark:border-amber-800 dark:bg-amber-950 dark:text-amber-300">
      ...
      <div className={kind !== "book" ? "hidden" : "flex flex-wrap items-end gap-3 rounded-xl border border-neutral-200 p-4 dark:border-neutral-800"}>
        <label className="text-copy">
          <span className="mb-1 block text-body text-neutral-500">List name</span>
        ...
        <label className="text-copy">
          <span className="mb-1 block text-body text-neutral-500">Whose book is this?</span>
        ...
        {partnerSel === "new" && (
          <>
            <label className="text-copy">
              <span className="mb-1 block text-body text-neutral-500">Partner name</span>
              <input name="newPartner" placeholder="e.g. Meridian" className={`${input} w-44`} />
            </label>
            <label className="text-copy">
              <span className="mb-1 block text-body text-neutral-500">Partner type</span>
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <button formAction={discard} formNoValidate className={buttonClass("subtle", "md")}>
          Discard upload
        </button>
