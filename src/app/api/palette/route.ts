...
export async function GET(req: NextRequest) {
  ...
  try {
    const [accounts, campaigns, motions, partners, pursuits] = await Promise.all([
      ...
      pool.query<{ id: string; legal_name: string; industry: string | null }>(
      ...
      pool.query<{ id: string; name: string; status: string; legal_name: string | null }>(
      ...
      pool.query<{ id: string; status: string; legal_name: string }>(
      ...
      pool.query<{ id: string; name: string; partner_type: string | null }>(
    ...
    /* db unavailable — an empty palette beats a 500 mid-keystroke */
  }
