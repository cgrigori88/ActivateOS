...
async function main(): Promise<void> {
  ...
  await scenario(hero.org_id, async (db) => {
    ...
    // POSTURE-AGNOSTIC: a v1 decision keeps the staging pointer inside its content, a v2 decision
    // carries lineage columns. Attention cares that the approved action is queued, not how the link
    // is recorded — which generation wrote it is a deployment setting.
    const stagedId = inForce.legacyStagedActionId
