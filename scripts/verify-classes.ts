...
export const SUITES: SuiteSpec[] = [
  ...
  },
  {
    name: "slice2",
    cls: "SEEDED",
    why: "SLICE 2 — PILOT ONBOARDING, REVERSIBLE INTAKE, ATTENTION SELECTION. Its exit criterion is operational rather than architectural: the owner must be able to run real pursuits without an engineer seeding, correcting or deleting rows, so this suite exercises the product paths. OPERATING CONTEXT: an organization carries its own provenance with NO DEFAULT, so one nobody classified stays unclassified rather than becoming PRODUCTION, and an unknown value is refused by the database. ORG SWITCHING: the cookie is a preference and membership is the grant -- the selection is re-validated against org_members on every read rather than only when it is set, a forged id degrades to the user's own membership, the ROLE follows the selected organization so a switch cannot carry another org's role, and the whole tree is revalidated so no segment keeps rendering the previous tenant. INTAKE runs the real three-file owner sequence: accounts with contacts, then CRM opportunities. Provenance comes from the operating context and the file never gets a vote; the REAL uploader replaces the literal 'web'; and re-importing the same file matches rather than duplicates, recording MATCHED_PREEXISTING, which is what later makes reversal safe. BATCH LINEAGE SURVIVES COMMIT -- the staged rows are still discarded, but the effects are not, which is the difference between an import that can be explained and three integers. FAILURE is durable and truthful: the failed status is no longer written inside the transaction the failure aborted (where it was refused with 25P02 and the real cause was lost), it is written on a separate connection so the caller's rollback cannot discard it, and a genuine commit failure is asserted to end FAILED carrying the ORIGINAL error. REVERSAL is proved branch by branch: the shared identity graph is retained BY RULE and reported rather than hidden, with a biting control that no cross-org reference count and no 'delete if unreferenced' path exists; org-scoped rows created solely by the batch are compensated; a fill-only update a person has since changed becomes ROLLBACK_CONFLICT and THE LATER WORK IS KEPT; an opportunity a pursuit now depends on is RETAINED_REFERENCED; the batch, its effects and every disposition remain auditable; and a second reversal reports rather than acts again. ATTENTION proves the token is evidence context and not authority: it verifies, a body edited to claim rank 1 does not, a stripped or foreign signature does not, the comparison is constant-time and the body is parsed only after it verifies. Redemption resolves identity and membership INDEPENDENTLY and then checks the token against them, re-checks the subject against this org, and does NOT recompute P2 -- what is persisted is what was rendered. The two positions are stored separately and asserted to be DIFFERENT NUMBERS (#3 of 18 by P2, 1st card on the page), with the surface, sort mode, comparison-set size and top-K needed to interpret either. Replaying a token is idempotent while a newly rendered surface may legitimately produce a new observation. Rendering mints and does not write, the ordinal comes from the FINAL composed list rather than the pre-composition order, and the primary CTA is a form because a GET also happens on prefetch, back/forward and a pasted link. It COMMITS its fixtures, so it runs on a disposable seeded clone",
    isolation: "SEEDED_CLONE",
  },
  {
    name: "p2-equivalence",
    cls: "SEEDED",
    why: "P2 BATCH-LOADING EQUIVALENCE (Slice 2). Slice 1 discovery measured P2's read at exactly 3N + 9 statements with no portfolio bound, because loadModeledImpact called getValueCase once per pursuit and each call issued three -- a pursuit row, the account's largest open opportunity, and that account's economic facts. The replacement changes only WHERE THE ROWS COME FROM: one bulk query each, partitioned in memory, handed to the SAME pure assembleCase and assembleDrivers the per-pursuit path uses, so every economic rule (the BASELINE exclusion, the interval subtraction, the defensibility test, the supersession and validity-window filter, the ladder ordering, the conflict rule, the tie collapse, the spread arithmetic) is executed by identical code rather than restated. A performance change to a certified computation is only safe if it is provably the same computation, so this suite compares NOTHING IN SUMMARY: it serializes every value case from both readers and requires byte equality, then serializes the whole ranking -- every subject, rank, score, band, signal component, ordering, deterministic tie-break, comparison-set size and D-018 withheld count -- and requires byte equality again. It carries a NEGATIVE CONTROL that flips one boolean and proves the comparison notices, so byte equality cannot pass vacuously. It then measures the statement shape directly by counting queries on the client, proving the count no longer scales with the portfolio, and repeats the measurement against synthesised portfolios rolled back afterwards, so 'constant' is evidence at real N rather than an assertion at N=11. It writes nothing and runs its comparisons inside READ ONLY transactions, but it plants synthesised pursuits for the scale measurement, so it runs on a disposable seeded clone",
    isolation: "SEEDED_CLONE",
  },
  {
    name: "pilot-evidence",
    cls: "SEEDED",
    why: "Pilot Evidence Foundation (Slice 1): the anti-regret pass that captures only what a real pilot would otherwise lose PERMANENTLY. Three gaps, each found by tracing rather than assumed. FIRST, provenance: data_environment already exists on 25 tables and lineage.ts already keeps provenance separate from learning eligibility via an explicit allow-list, but the vocabulary could not say PILOT or CERTIFICATION -- and that had already begun to cost us, because 18 invocations including 4 of the 7 P8-0 observations are flagged PRODUCTION by a hardcoded /api/mcp default, and PRODUCTION is the sole learning-eligible environment. The suite proves the two dimensions stay separate: PILOT is real-world evidence and is deliberately NOT learning-eligible, because being real does not grant a training licence; CERTIFICATION is neither; DEMO is never; PRODUCTION remains both as the control that stops either filter passing by refusing everything; and the two SQL fragments are genuinely different, with PILOT present in one and absent from the other. It proves the widened CHECK accepts the new words while still refusing an unknown one, so the constraint was widened rather than removed. SECOND, the legacy certification manifest: a gate row keeps its ORIGINAL data_environment because history is never rewritten, and is nonetheless excludable deterministically by exact id rather than by timestamp or fixture-name folklore, with duplicate entries refused. THIRD, decision-time attention, which is the one fact unrecoverable at any price: P2 rank is computed at read time, never persisted, and depends on a comparison set that moves. The producer is EXPLICIT and runs on write paths only -- capturing during a Today render would reintroduce exactly the defect D-HIST-1 found and 6f3e65d closed, pipeline_snapshots written from a page render whose own comment read 'history accrues just by looking', which is why 97e975f0 could not be safely reconstructed during the P45-4 incident. It is self-deduplicating on a fingerprint of the ranking INPUT STATE, so an unchanged ranking writes nothing and there is no cadence to decide, while a REORDERING or a different algorithm version is a different state and IS captured -- the control that stops the dedup from silently swallowing real change. It proves rank, score, band and comparison-set size are preserved because a rank is meaningless without the set, that components carry only declared keys and numbers, and that topReasons prose never reaches the table. Finally it proves the refusals structurally: no label, outcome, reward or causality column or table exists, and PILOT was not quietly added to the learning allow-list. Append-only is proved by PRIVILEGE -- both new tables end at app_rw=ar like change_ledger, with RLS enabled and FORCED -- and the mutable outcome tables are named as non-evidentiary rather than quietly relied upon. THE CORRECTION PASS added the four things the first version left as prose. ELIGIBILITY IS NOW A COMPOSITION, and the suite demonstrates the defect before it demonstrates the fix: the learning allow-list ALONE admits a known mislabelled certification row, because saying PRODUCTION is exactly what is wrong with it, and only learningCorpusSql refuses it -- flanked by two controls, an identical PRODUCTION row that is NOT in the manifest and is still admitted, and a DEMO row refused although the manifest never mentions it, so neither half can be passing by refusing everything. PROVENANCE IS DERIVED FROM THE SUBJECT rather than defaulted: recordChange ends in `?? PRODUCTION`, which is how the hosted project came to hold change_ledger rows marked PRODUCTION whose pursuit is DEMO, in the one store that cannot be corrected by UPDATE; both opportunity emissions now read pursuits.data_environment server-side and the suite asserts the whole ledger trail of a DEMO pursuit is DEMO. CREATION IS DISTINGUISHED FROM IMPORT: promoting an ACTIVE motion originates a deal here and now, so it emits OPPORTUNITY_CREATED with an absent before_state, while the CRM import path records a crm_snapshots observation and deliberately claims no creation, because import time is not business creation time and no imported column carries the real one. THE AMOUNT-ONLY EVENT IS PROVED UNREACHABLE rather than instrumented -- no application path mutates amount_usd after creation, and a raw UPDATE is shown to produce no history at all, which is why no such path may exist. It also proves the observation is server evidence: a dispatch that fails after staging leaves NO orphan (staged, then rewound to zero through a savepoint), the producer's input type accepts no caller-supplied rank, score, fingerprint or eligible set, the fingerprint is computed from the server-ranked view, the entire P2 ranking completes inside a READ ONLY transaction so the database itself enforces that looking writes nothing, and no entry point reads dataEnvironment from a request body, params or tool arguments. Every structural assertion scans source with COMMENTS STRIPPED, after two of them first failed against the prose explaining the very invariant they check. THE PROVENANCE PASS then closed the ambient-PRODUCTION defect at its source. A credential now carries its own data environment (0120), because on the MCP path the credential is the ONLY thing a caller cannot choose: it is resolved through a SECURITY DEFINER function app_rw cannot bypass, and body, params, arguments and headers are all caller-controlled and therefore worthless as provenance. The suite proves a certification key resolves CERTIFICATION and a pilot key PILOT, that each reaches the invocation row unaltered, that a foreign-org key lends neither its org nor its provenance, and that resolution is a property of the bearer alone. It proves the column has NO DEFAULT by minting through a path that never names it -- an earlier version tested only an explicit null and would have let `default 'PRODUCTION'` back in silently -- and that provenance is IMMUTABLE by privilege, app_rw holding update on revoked_at and nothing else, with revocation kept as the positive control so the check does not pass by forbidding everything. It proves omission is now impossible rather than merely discouraged: DispatchCtx.dataEnvironment and ChangeEvent.dataEnvironment are required, no `?? \"PRODUCTION\"` survives on either path, /api/mcp no longer contains the literal at all and fails closed on an unclassified credential, and the five copy-pasted defaults in the pursuit surfaces became one resolver that returns null so the surface REFUSES instead of substituting. Finally it enforces the owner ruling on attention: a real recommend_pursuit_plan@1 dispatch writes ZERO attention observations, because that capability never consumed P2 and a ranking attached to it would be false lineage -- so wiring the producer there fails this suite. It COMMITS its fixtures, so it runs on a disposable seeded clone",
    isolation: "SEEDED_CLONE",
  },
  {
    name: "p8-0",
