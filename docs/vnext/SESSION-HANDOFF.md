...
| **Exact next step** | **Resolve D-G5-1 (local code change, then its own review and deploy approval):** add deterministic tiebreakers.<br>• `src/lib/context/divergence.ts` "stage vs engagement": add an explicit ORDER BY, e.g. `o.updated_at, o.id`.<br>• `src/lib/lifecycle/projection.ts` list name: `order by pm.company_id, ap.created_at, ap.id`.<br>Certify locally, then deploy to the branch Preview and re-crawl. **Only then Gate 7** (hosted tenant/RLS certification).<br>D-P1 (`?timeframe=` snapshot overwrite) stays a Gate 9 / pilot blocker.<br>Baseline: migrations 105, manifest `db1f78f7a11bbacb`, business-data `c9623fb5abe2f9bc`, whole-world `dce27935d88743fb`, security hash `30772757ebd4688c`; CFR-1. |
