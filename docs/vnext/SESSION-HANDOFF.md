...
| **Exact next step** | **Gate 1b (owner-approved, a hosted mutation):** apply `0104_h1b0_consent_scoped_access.sql` to `mejokqxriwyawfhawuxu` **only** (never `qifatlqxfuhwrwvpbwsc`). Then re-run the Gate 1 read-only checks against the re-baselined manifest `db1f78f7a11bbacb` and fingerprint `2678f34d4fc7b0a2`: migrations 104, guard on 8 tables, 14 functions `app_rw`-only. Gates 2–9 follow, each separately approved. |
