...
| **Exact next step** | **Gate 5 (owner-approved separately; Vercel Preview, this branch only):** **change** Preview `DATABASE_URL` for `roadmap/pursuitos-vnext` to the `app_rw` pooler string (`app_rw.mejokqxriwyawfhawuxu` on :6543); leave `DATABASE_URL_OWNER` as is; redeploy.<br>• Pass: the app boots; every room renders.<br>• Rollback: the Gate 8 procedure.<br>Baseline: migrations 105, manifest `db1f78f7a11bbacb`, business-data `c9623fb5abe2f9bc`, whole-world `dce27935d88743fb`, security hash `30772757ebd4688c`; comparisons follow **CFR-1** (`H1-PRE-PILOT-HARDENING.md` § "Gate 4 close-out"). The Gate 5 inputs, including `APP_RW_PASSWORD`, must be loaded in the launching shell. |
