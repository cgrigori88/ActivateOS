...
| **Exact next step** | **First: an owner decision on the proposed CFR-1.1.** The hosted manifest's `days_since_activity` is clock-relative: the digest moved `db1f78f7a11bbacb` → `14e2e97f8453fb75` at 21:02Z with no data change (all 155 per-table fingerprints identical). See `H1-PRE-PILOT-HARDENING.md` § "Gate 7 closeout note".<br>**Then Gate 8 — rollback rehearsal (two separate owner approvals):**<br>(1) set the branch-scoped Preview `DATABASE_URL` (`m6TuSKisz54kJlsD`) back to the owner connection by a value-only update, leaving `DATABASE_URL_OWNER` as is; redeploy; confirm `/api/build` reports `postgres` / bypassRls true / tenantEnforcement false and the crawl is identical;<br>(2) re-apply the `app_rw` value; redeploy; confirm `/api/build` reports `app_rw` / false / true and the crawl is identical.<br>CFR-1 applies. **D-P1 stays OPEN** and must be fixed before Gate 9 / a real pilot.<br>Baseline: migrations 105, manifest `db1f78f7a11bbacb`, business-data `c9623fb5abe2f9bc`, whole-world `dce27935d88743fb`, security hash `30772757ebd4688c`. |
