...
| **Gate 1 (2026-09-15T02:40Z)** | **Read-only preflight of `mejokqxriwyawfhawuxu`: FAIL as specified, on the manifest criterion only.**<br>• Hosted digest `db1f78f7a11bbacb` vs documented `be0da833990ce436`. One extra `stakeholders` row: the economic buyer verified during the owner's Slice 2A hosted acceptance, alongside the plan decision and review.<br>• All other checks pass: identity, 103/103 migrations, `app_rw` NOLOGIN / no bypass, RLS+FORCE 155/155, catalogue identical to local, 0 digests / routines, no orphan or org-less anomalies, canonical counts exact, 0 send rows.<br>• Zero writes proven. Hosted fingerprint `2678f34d4fc7b0a2`.<br>Record: `H1-PRE-PILOT-HARDENING.md` § "Gate 1 — RESULT" |
| **Exact next step** | **Owner decision on the Gate 1 manifest criterion:** (a) re-baseline the hosted expected digest to `db1f78f7a11bbacb` / fingerprint `2678f34d4fc7b0a2` (recommended; documentation only), or (b) reseed the hosted world to canonical (a hosted write, separate approval). After that, do local work item **H1B-0**: consent flows under `app_rw` and the `/api/build` posture probe. **Gate 1b and Gate 5 must not run before H1B-0 is certified.** |
