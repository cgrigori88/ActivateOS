...
| **Exact next step** | **Gate 4 (owner-approved; a Vercel Preview change for this branch only):** **add** `DATABASE_URL_OWNER` = the current owner string, leaving `DATABASE_URL` untouched, then redeploy the Preview.<br>• Pass: the build is healthy; `/api/build` still reports `postgres`; the owner paths work (login, join, admin members, webhook); a full room crawl matches the pre-change crawl.<br>• Rollback: remove the variable and redeploy.<br>No database change. The baseline: migrations 105, manifest `db1f78f7a11bbacb`, business-data fingerprint `79321d9130d1dc94`, whole-world fingerprint `de05e204801988d1`, `app_rw` LOGIN true. |
