...
| **Exact next step** | **Gate 8 Phase 2 — re-apply `app_rw` (separate owner approval; `APP_RW_PASSWORD` must be loaded in the launching shell).** Follow the 9-step procedure in `H1-PRE-PILOT-HARDENING.md` § "Gate 8 Phase 1", in outline:<br>• build the `app_rw` value in memory and probe it;<br>• value-only PATCH of `m6TuSKisz54kJlsD`, then redeploy;<br>• `/api/build` must report `app_rw` / false / true / live;<br>• owner paths and the 37-room crawl twice, compared with the Gate 7 `app_rw` crawl allowing only time-derived text;<br>• DB under CFR-1.1;<br>• push the local docs commits.<br>Then fix D-G8-1 (stakeholder ORDER BY) and **D-P1** before Gate 9.<br>Baseline: migrations 105, business-data `c9623fb5abe2f9bc`, whole-world `dce27935d88743fb`, security hash `30772757ebd4688c`, manifest under CFR-1.1. |
