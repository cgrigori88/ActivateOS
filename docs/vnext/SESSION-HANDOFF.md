...
| **Gate status** | **H1A COMPLETE (local)** · Gate 1 PASS (re-baselined) · H1B-0 COMPLETE (local) · **Gate 1b PASS** · Gate 2 not begun · **H1B IN PROGRESS** — readiness reviewed, nine gates defined (`H1-PRE-PILOT-HARDENING.md` § H1B execution plan). H1 NOT complete until H1B passes hosted certification |
...
| **Exact next step** | **Gate 2 (owner-approved, a hosted role change):** `alter role app_rw with login password '<operator-generated secret>'`, run as `postgres` on `mejokqxriwyawfhawuxu` **only**. The secret is stored in the operator's secret store and never printed. Pass: `rolcanlogin=true`, nothing else about `app_rw` changed. Rollback: `alter role app_rw nologin`. Then Gate 3: prove the `app_rw.<ref>` pooler login. Hosted baseline after 1b: manifest `db1f78f7a11bbacb` / fingerprint `0288ae73bb385a1c`. |
