...
| **Exact next step** | **Gate 6 (`/api/build` role and RLS posture proof; no mutation):** on the current branch Preview, require `database.role=app_rw`; a live probe showing `current_user=app_rw` and `rolbypassrls=false`; the project ref parsed; and the build SHA being the approved commit.<br>Before Gate 7: resolve **D-G5-1** (deterministic tiebreakers in `divergence.ts` and `projection.ts`), or document the set-based comparison for those rooms. Before Gate 9: resolve **D-P1** (`?timeframe=` snapshot overwrite).<br>Baseline unchanged: migrations 105, manifest `db1f78f7a11bbacb`, business-data `c9623fb5abe2f9bc`, whole-world `dce27935d88743fb`, security hash `30772757ebd4688c`; CFR-1. |
