...
| **Exact next step** | **An owner decision on D-G8-2** (the latent ordering backlog in `H1-PRE-PILOT-HARDENING.md` § "Ordering audit"): fix its **high**-priority default-view rows before Gate 9, as one tie-break-only change certified the D-G5-1 / D-G8-1 way, or accept them with a plan. **Then fix D-P1** (`/pipeline?timeframe=` overwrites the canonical daily snapshot) before Gate 9 / a real pilot.<br>Baseline: migrations 105, business-data `c9623fb5abe2f9bc`, whole-world `dce27935d88743fb`, security hash `30772757ebd4688c`, manifest under CFR-1.1. |
