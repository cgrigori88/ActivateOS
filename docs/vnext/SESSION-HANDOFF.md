...
| **Exact next step** | **Gate 7 (owner-approved separately): hosted tenant/RLS certification** on the branch Preview (`app_rw` runtime). It covers:<br>(i) catalogue checks;<br>(ii) a room crawl compared against the **D-G5-1 accepted crawl (`1c4fb5e`)**, not the pre-fix Gate 4 crawl;<br>(iii) the READ_ONLY / ROLLBACK_SAFE suites with `DATABASE_URL_VERIFY` = the `app_rw` string;<br>(iv) a two-org blind test;<br>(v) the partnership and joint handshakes;<br>(vi) hosted human review.<br>CFR-1 applies. **D-P1 (`?timeframe=` snapshot overwrite) stays OPEN** and must be fixed before Gate 9 / a real pilot.<br>Baseline: migrations 105, manifest `db1f78f7a11bbacb`, business-data `c9623fb5abe2f9bc`, whole-world `dce27935d88743fb`, security hash `30772757ebd4688c`. |
