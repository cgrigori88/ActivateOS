...
| **Gate status** | **H1A COMPLETE (local)** · Gate 1 PASS (re-baselined) · H1B-0 COMPLETE (local) · **Gate 1b PASS** · **Gate 2 BLOCKED at precheck (not executed)** · **H1B IN PROGRESS** — readiness reviewed, nine gates defined (`H1-PRE-PILOT-HARDENING.md` § H1B execution plan). H1 NOT complete until H1B passes hosted certification |
...
| **Exact next step** | **Owner decision:** either write hardening migration **0105** (`ALTER FUNCTION … SET search_path = pg_catalog, public, pg_temp` on the 27 functions listed in the H1 doc; recommended), or explicitly accept the residual risk in writing.<br>If 0105: certify it locally (a new pg_temp-shadowing negative test in `partnership-app-rw`, `certify-world --runs 2`, the rehearsal), then apply it to `mejokqxriwyawfhawuxu` as its own approved step.<br>Then re-run Gate 2 with `APP_RW_PASSWORD` loaded into the launching shell via hidden input (`read -rs APP_RW_PASSWORD && export APP_RW_PASSWORD`) and the session relaunched. Never paste it into chat. |
