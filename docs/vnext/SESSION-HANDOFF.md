...
| **P45-1 HOSTED ACCEPTANCE (2026-09-16)** | **HOSTED ACCEPTED / CLOSED — and P45-D1 CLOSED.**<br>• **Phase 1:** `d742a77` → **`dpl_2syZQr94SCB2kyPpCs1vJN594BNV`**; 0/159 moved on code deploy; **append-only proven hosted 9/9** — `app_rw` CAN insert via corrected `recordChange` (row **born** linked), **CANNOT** update or delete. No permission added.<br>• **CFR-1.2 adopted + crawl re-baselined:** clock-derived differences permitted only when recomputable from unchanged source and a **named** time expression; two-class reporting (**STRICT** zero-tolerance / **CLOCK-DERIVED** itemised); DB fingerprints stay exact. 30 differences classified: 23 day counters, 6 from `Hist · DR modernization` ($610,000) leaving the 90-day won window (1,780,000−610,000=1,170,000; 4−1=3), 1 invite age recomputed as exactly 2.<br>• **Phase 2: 48/48** on a fully synthetic world (**Globex untouched**), every execution via **`withTenantOrg` on real `app_rw`**: two-gate control · wrong principal REFUSED · no grant REFUSED · grant does NOT override permission · **ONE EXECUTED invocation, ONE draft (`status=draft`), no outbox** · full audit chain · idempotent replay · pause/resume from durable state · **PLAN_SUPERSEDED, never retargeted** · suspend/revoke refused.<br>• Refusals also record **REJECTED** invocations — the audit trail working; uniqueness applies to the **EXECUTED** one (28→29).<br>• **Cleanup:** every row by exact id, dependency order, no wildcards. **159/159 fingerprints restored exactly**; post-cleanup crawl RAW digest **byte-identical**. **No trace left.**<br>• Regression: p45 50/0 · persisted 17/0 · semantic 50/0 · dg85 19/0 · dp1 26/0 · partnership 117/0 · tenant-isolation 205/0 · search-path 39/0 + 12/0 (**31/0**) · rehearsal 38/38+6/6.<br>• Hosted record unchanged: **109** · `6abe424f43bff901` · `c299c6e372c686c4` · `569e5497a7622048` · 31/0. controlPlane **OFF** throughout; runtime tables 0/0/0/0.<br>**P4 substrate and P5 execution both PROVEN HOSTED. Slice 2 NOT STARTED.**<br>Record: `P4-P5-GOVERNED-RUNTIME.md` §14–16 |
| **Exact next step** | **Slice 2 — the approval workflow**, on the owner's scope instruction. `WAITING_FOR_APPROVAL` is schema-supported but unexercised, and `actor_capability_grants.approval_required_override` exists nullable and unused precisely so Slice 2 needs no migration. Baseline: migrations **109** · business `6abe424f43bff901` · world `c299c6e372c686c4` · security `569e5497a7622048` · 31/0 · `controlPlane` OFF. Then Slice 3 (multi-step plans — the one real P3 change). **Not started.** |
| **Superseded next step** | **Re-run the hosted P45-1 FUNCTIONAL gate** with the P45-D1 correction pushed — separately authorised. Use the Option-A synthetic fixture strategy (disposable org whose approved action genuinely means 'draft a campaign touch'), execute through the real `app_rw` identity, and restore all 159 fingerprints exactly. Hosted baseline: migrations **109** · business `6abe424f43bff901` · world `c299c6e372c686c4` · security `569e5497a7622048` · 31/0. Then Slice 2. |
