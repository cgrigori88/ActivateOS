...
| **Exact next step** | **Hosted D-G5-1 acceptance (owner approval required):**<br>• push `roadmap/pursuitos-vnext` (auto-deploys the branch Preview on the `app_rw` runtime; no env change);<br>• confirm `/api/build` reports `app_rw` / false / true on the new commit;<br>• run the signed-in 37-room crawl twice, requiring Today's capped "stage vs engagement" order to be longest-quiet first (Datacenter exit, Kubernetes, Core banking, Legacy) and the CDW `/pipeline` label to be "CDW customer book", stable across the crawls;<br>• run the DB check under CFR-1.<br>**Only then Gate 7.** D-P1 remains a Gate 9 / pilot blocker.<br>Baseline: migrations 105, manifest `db1f78f7a11bbacb`, business-data `c9623fb5abe2f9bc`, whole-world `dce27935d88743fb`, security hash `30772757ebd4688c`; CFR-1. |
