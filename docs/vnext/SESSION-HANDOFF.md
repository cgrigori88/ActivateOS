...
| **Gate status** | **H1A COMPLETE (local)** · Gate 1 PASS (re-baselined) · H1B-0 COMPLETE (local) · **Gate 1b PASS** · Gate 2 precheck BLOCKED (superseded) · **H1B-0.1 COMPLETE (local)** · **Gate 1b.1 PASS** · **Gate 2 PASS** · **H1B IN PROGRESS** — readiness reviewed, nine gates defined (`H1-PRE-PILOT-HARDENING.md` § H1B execution plan). H1 NOT complete until H1B passes hosted certification |
...
| **Exact next step** | **Gate 3 (owner-approved; no hosted mutation, a rolled-back probe):** prove the pooler login from the operator machine as `app_rw.mejokqxriwyawfhawuxu` on the transaction pooler :6543, with the same `APP_RW_PASSWORD` loaded via hidden input (never pasted into chat).<br>• Expect `current_user=app_rw` and `rolbypassrls=false`.<br>• With no `app.org_id`, every tenant table returns 0.<br>• With `app.org_id` = Vertex, counts equal the owner's.<br>• A write carrying another org is refused.<br>• Then rollback, with no Vercel change.<br>STOP if the login is refused (see the Gate 3 row of the H1B plan). The baseline to check against: migrations 105, manifest `db1f78f7a11bbacb`, business-data fingerprint `79321d9130d1dc94`, whole-world fingerprint `de05e204801988d1`, `app_rw` LOGIN true. |
