...
SLICE 2B Pursuit Attention + Today/Queue   (P3)        DEMO CERTIFIED / FROZEN
   ...
   ↓
H1  PRE-PILOT HARDENING GATE               (P6 / #67)  H1A · GATE 1 · H1B-0 · GATE 1b PASS · H1B-0.1 · GATE 1b.1 PASS · GATE 2 PASS · GATE 3 PASS · GATE 4 PASS (re-baselined) · GATE 5 PASS · GATE 6 PASS · D-G5-1 CLOSED · GATE 7 PASS · GATE 8 PASS · D-G8-1 CLOSED · **D-G8-2A HOSTED ACCEPTED / CLOSED** (`a5da3b2` as `dpl_BcKULAScmeWVZaQYkakWbCRiZw7w`; closure `94491ea1…`, 333 files; 173 → 63, in-boundary unresolved = 0; 37/37 rooms and 4/4 passes byte-identical; vs the accepted D-G8-1 crawl 32/37 identical, 5 pure reorders, 0 membership changes; DB 154/155 identical with only the documented `pipeline_snapshots` new-date row, not re-baselined) · D-G8-2B · **D-G8-3 HOSTED MIGRATION GATE PASS (0106 + 0107)** · **D-G8-3A/B/D HOSTED ACCEPTED / CLOSED (`21326e5`)** · **D-G8-3C → D-G8-4D** · **D-G8-4A/B/C/D HOSTED ACCEPTED / CLOSED** (`982a01f` + the 4C correction `48a6157`) · **D-G8-5 HOSTED ACCEPTED / CLOSED (0108)** · **D-P1 FIXED LOCALLY / NOT PUSHED** (code-only, no migration — repo stays at 108; `upsertCanonicalPipelineSnapshot(db, orgId)` takes no caller-computed value, so no caller-side narrowing — `?timeframe=`, the ecosystem scope, or any future filtered projection — can reach canonical history. One defect class, FILTERED-VIEW SNAPSHOT POISONING, widened at implementation and owner-ACCEPTED; `dp1-snapshot-boundary` 26/0 with a negative control reproducing the pre-fix poisoning; `certify-world --runs 2` 90 clean, digest `e98b43254f98d5ec` unchanged) · **D-P1 HOSTED ACCEPTED / CLOSED** (`f936fbe` as `dpl_FnUKb4vWHMQkpAKAbxtDxCKeyyYp`; timeframe 7/30/90, narrowed ecosystem scope and the combined filter all leave the canonical row byte-identical at `9b2c3c6ce81d9f12`; 8 repeated filtered reads → ONE hash; concurrency leaves canonical state; crawl 37/37 ×4 and 37/37 identical to the accepted D-G8-5 crawl; 0 of 155 fingerprints moved) · **PRE-GATE-9 HARDENING COMPLETE** · **GATE 9 — PASS** (pilot-readiness decision gate, 2026-09-16: baseline re-verified with 0 of 155 fingerprints moved; final regression clean; crawl 37/37 ×4 identical to both accepted crawls; **PILOT BLOCKERS: NONE**) · **H1 PRE-PILOT HARDENING — COMPLETE** · **PILOT READINESS — ACCEPTED** → **amended P0–P10 roadmap: P4 / P5 transition + continued P3 depth**
P45  GOVERNED PURSUIT RUNTIME (P4 identity + P5 run)        **SLICE 1 IMPLEMENTED LOCALLY / NOT PUSHED** — migration 0109 (additive, no SECURITY DEFINER, 31/0 preserved); `governed_actors` + `actor_capability_grants` answer WHO may act, `pursuit_runs` + `pursuit_run_steps` make one governed invocation durable and resumable; `dispatchSkill` remains the ONLY consequential-action boundary and the grant check is additive (a grant cannot override `required_permission`); a run is permanently pinned to its plan revision and CANCELLED as PLAN_SUPERSEDED rather than retargeted; request-triggered under `app_rw` + `withTenant` (worker drain deliberately deferred — the owner pool must not become ambient runtime authority); double-gated behind `VNEXT_CONTROL_PLANE_ENABLED` + `org_features.governed_action`, default OFF; `p45-runtime` 45/0; `certify-world --runs 2` digest `d1f970a533dbee1a` stable; send 0/0/0/0/0 · **MIGRATION 0109 HOSTED ACCEPTED** (`2f16091` as `dpl_8MDMbJwHiKePAExe6xRifxhfsASh`; level 109; RLS+FORCE ×4 tenant-scoped; cross-org refused relationally; existing actor_id contracts preserved; zero backfill; policies 380→384, grants 620→636 with 0 existing changed, functions/triggers/roles/protected identical; new security `569e5497a7622048`, world `c299c6e372c686c4`, business-data `6abe424f43bff901` — its movement PROVEN to be column-addition only, no business row changed; flag-OFF crawl 37/37 byte-identical to Gate 9; runtime tables EMPTY; send 0/0/0/0/0) · **P45-D1 HOSTED CORRECTED / CLOSED** (ledger linkage written atomically on INSERT; append-only preserved, no permission added; proven under real `app_rw`) · **P45-1 HOSTED ACCEPTED / CLOSED** (`d742a77` as `dpl_2syZQr94SCB2kyPpCs1vJN594BNV`; synthetic Option-A fixture; 48/48 — two-gate control, principal matching, grant enforcement, ONE draft only, idempotent replay, pause/resume, PLAN_SUPERSEDED, suspend/revoke, all via `withTenantOrg` on real `app_rw`; **159/159 fingerprints restored exactly**; post-cleanup crawl byte-identical; send 0/0/0/0/0) · **CFR-1.2 adopted** (clock-derived comparison rule; STRICT vs CLOCK-DERIVED classes) · **P45-2 APPROVAL LIFECYCLE — HOSTED ACCEPTED / CLOSED** (migration 0110; 60/60 hosted, 56/56 local; request→approve/reject/invalidate, concurrency single-winner (approve/approve, approve/reject, approve/invalidate hosted; reject/reject local only, closed on the decision-value-agnostic one-terminal invariant), append-only history `app_rw` cannot rewrite, full audit chain; **160/160 fingerprints restored**; CFR-1.2 STRICT-clean; production human approval identity **NOT** proven — tracked) · **P45-3 SEQUENTIAL MULTI-STEP RUNTIME — IMPLEMENTED LOCALLY / NOT PUSHED** (migration 0111 = one ledger word `RUN_STEP_COMPLETED`, local only; an ordered program born atomically, identified as a whole, advanced ONE step per request, halting rather than skipping, re-authorized at every step boundary, with one approval releasing exactly one step; `p45-program` **78/0**, negative control drives 20+ checks red; digest `f72d1ff0d6b07b42` unchanged) · **P45-3-D1 CORRECTED LOCALLY** — a resume may advance only the run-step generation it observed when the request began, so duplicate/concurrent requests cannot authorize two sequential consequential actions; no lease, no migration, 0111 unchanged) · **P45-3 HOSTED ACCEPTED / CLOSED** (hosted migration **111**; gate 24/24, functional 75/75; generation-safe advancement, per-step approval isolation and partial-execution semantics all proven hosted; **160/160 fingerprints restored**, post-cleanup crawl byte-identical; new record business `6db00c6841c51fc3` · world `f2d83f71489c6682` · security `092a20af64a0444e` · 31/0) **P45-4 — AGENT ACTOR EXECUTES A GRANTED CAPABILITY — HOSTED ACCEPTED / CLOSED** (2026-09-20): a production AGENT credential resolves to a durable governed actor holding an exact live `draft_campaign_touch@1` grant, and one real same-org consequential action executes through `dispatchSkill` with immutable actor + credential + grant attribution. Closed the gap where `/api/mcp` dispatched AGENT writes with no governed actor, so agent authority came from a key scope rather than a grant. Migration 0116 additive; strict enforcement live on `dpl_7P8gX9…`/`7c4aea5`/schema 116 with `dpl_3CiYuRLR…` as the sole security rollback; security-weaker Preview execution surfaces 0; `p45-4` 52/0. No model/token/cost evidence claimed. Operational incident `D-P454-PRODUCTION-ARTIFACT-DELETION` recorded separately (12 historical Production artifacts deleted; live Production kept serving; no Production data modified) with follow-up `D-PROD-ROLLBACK-ARTIFACT-RETENTION` OPEN. · **Still open after P45-4 — WORKER/SYSTEM governance remains ungoverned legacy, and the sequential runtime substrate claims remain as before:** plan-derived program synthesis (the genuine P3 change) · DAG / parallel execution · worker drain · production human approval identity · then Slice 2 (approval workflow), Slice 3 (multi-step plans, the one real P3 change)
   ...
   ↓
SLICE 2C Coordination breadth              (P3)        **2C-A HOSTED ACCEPTED / CLOSED** — ordered plan content: migration 0115 (composite candidate key on `pursuit_plan_revisions`, nullable `plan_revision_id`/`plan_action_key` on `motion_actions`, tenant-composite FK with `ON DELETE SET NULL (plan_revision_id)`, one-directional live-lineage CHECK, partial unique index); `PLAN_CONTENT_SCHEMA_V2` with up to 3 ordered actions, dispatched by stored schema and never by shape; v1 immutable and read for life by v1-to-v1 fingerprints; `actions[0]` is the approved primary and the current actionable item is separately derived, so REVIEW_NEEDED never rewrites what was approved; live lineage lives on `motion_actions` columns, not in plan content, and PlanContent carries **no execution authority**. Rolled out behind `PLAN_CONTENT_V2_WRITES_ENABLED` (absent ⇒ false): OFF-posture certification **57/0** with zero hosted v2 revisions, then activation on `dpl_J4d7ynXoZ4PGNV42NqWoRgHP4kWk` / source `89ed7001` / schema 115. **First hosted v2 revision `3d94daee…` at 2026-09-20T04:25:21.481Z permanently retires `e55499b` as a rollback target**; canonical post-v2 rollback is `dpl_3hXGEzo6njtV7VDhJHVQB8fqtYL9` (gate false), certified after real v2 data existed. Final state after cleanup: pursuits 14 · motions 7 · queue 6 · revisions 3 all schema 1 · live lineage 0 · ledger 68 · P45 0, one accepted movement of **+13 `governed_action_invocations`** fully classified; world `f716178809021a09`. §16M and §16N adopted. **2C-B — Plan → Runtime Compilation: NOT STARTED / DEFERRED.** Remaining 2C breadth: goal editing · plan closure · multi-pursuit plans · review recording on events
   │      (2C-A contract: `docs/p3-slice-2c-ordered-plan-actions-contract.md`)
   ↓
SLICE 3  Portfolio Pertinence              (P2)   **HOSTED ACCEPTED / CLOSED** (Phase 2 16/16, Phase 3 65/66; D-P2-1 and D-P2-2 hosted closed) — five declared signals, deterministic, no model, no persistence, no migration;
