...
| **vNext isolated database** | **BLOCKED** — needs a **working credential**, not a decision and no longer an execution context | target ref `mejokqxriwyawfhawuxu` | 2026-09-14 | **Reset the database password** (Supabase → project `mejokqxriwyawfhawuxu` → Settings → Database → Reset database password), re-export `DEMO_TARGET_URL`, then run the unchanged 5-command sequence in `ENVIRONMENT-MAP.md` §10: migrate → mark synthetic → read back → seed → reconcile | **Egress is resolved** — from a laptop the host, both pooler ports and the direct host all connect, so the Claude Code Web port policy was never the whole story. The credential is **rejected `28P01` at all three endpoints**, one of which bypasses Supavisor, so the project resolves and only the password is wrong. `environment_identity` therefore remains **UNREADABLE** (still a connection fact, not a marker fact) and the schema/seed state is still unknown. Identity **proven distinct** from `qifatlqxfuhwrwvpbwsc`. Nothing written |
| vNext isolated preview | **NOT STARTED** (blocked upstream) | — | 2026-09-14 | The isolated database first (row above), then `PREVIEW-ISOLATION-PLAN.md` Objective C Option 2 step 5 — the one additive Vercel env var | Nothing built. No Vercel scope has been touched in any session. Owning an isolated database does not by itself isolate Preview |
