...
**Last updated:** 2026-09-12T10:40Z
**Lane:** `roadmap/pursuitos-vnext` @ `e0365d4` + GATE C review package (Slice 1 chunks 1–6B complete; **GATE C prepared — Slice 1 NOT promotable, see N-1**)
...
| · pursuit context narrative (rendered) | P1 | **BLOCKED** (was PREVIEW READY) | `99bd5dd` `components/pursuit/context-narrative.tsx` | 2026-09-12 | Resolve GATE C **N-1** before any promotion | First rendered vNext surface. Flag-gated; OFF is byte-identical to pre-6B. **GATE C found `Earlier history (7 more)` renders no events — 7 of 10 ledger rows, incl. the demo's §2 `PARTNER_OVERRIDE`, are unreachable flag-ON.** Plus N-6: a 593px column void. See `GATE-C-PRODUCT-REVIEW.md` |
...
| **GATE C findings N-1 … N-6 (NEW)** | `docs/vnext/GATE-C-PRODUCT-REVIEW.md` §8. **N-1 is a promotion blocker**: the `Earlier history (7 more)` disclosure renders no events and points at an "activity record" that the merge removed, so 7 of 10 `change_ledger` rows — including the demo's §2 `PARTNER_OVERRIDE` — are unreachable with the flag on. **N-6**: the merge leaves a 593px void beside the new panel and orphans "Outcome & attribution". N-2…N-5 are lower severity. **None fixed** — GATE C was preparation only. |
