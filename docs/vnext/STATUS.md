...
**Last updated:** 2026-09-15 (H1B Gate 3 — `app_rw` pooler login and hosted RLS / tenant-context proof, PASS; Gate 4 not begun)

**2026-09-15 (latest) — H1B GATE 3: PASS.** This was a probe on the isolated vNext database `mejokqxriwyawfhawuxu`, with no hosted mutation: every `app_rw` transaction was rolled back, and the owner was read-only.
...
| **H1B — Least-privilege runtime / RLS cutover** | P6 / #67 | **IN PROGRESS** — Gate 1 PASS (re-baselined) · H1B-0 COMPLETE · Gate 1b PASS · **Gate 2 BLOCKED / NOT EXECUTED** · **H1B-0.1 COMPLETE (local)** · **Gate 1b.1 PASS** (0105 hosted; 31/31 hardened, 0 unsafe) · **Gate 2 PASS** (`app_rw` LOGIN false → true; nothing else changed) · **Gate 3 PASS** (pooler login proven; RLS exact on 155 tables for no context and 3 orgs; no context leak; foreign writes refused; zero residue) | `roadmap/pursuitos-vnext` | 2026-09-15 | Gate 4 (Vercel Preview: add `DATABASE_URL_OWNER`, owner-approved) → Gates 5–9 | D-045, D-048, D-049, D-050. Hosted baseline: migrations 105 · manifest `db1f78f7a11bbacb` · business-data fingerprint `79321d9130d1dc94` · whole-world fingerprint `de05e204801988d1` · `app_rw` LOGIN true (not yet used) |
