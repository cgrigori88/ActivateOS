...
**Last updated:** 2026-09-16 (**D-P1 FIXED LOCALLY / NOT PUSHED** — the canonical snapshot writer; hosted D-P1 acceptance is the FINAL pre-Gate-9 gate; **D-G8-5 HOSTED ACCEPTED / CLOSED** — migration 0108 applied hosted; **D-P1 is the FINAL pre-Gate-9 blocker**; **D-G8-4 OVERALL HOSTED ACCEPTED / CLOSED**; earlier **D-G8-3A/B/D — HOSTED ACCEPTED / CLOSED**; D-G8-3C → **D-G8-4D OPEN**; earlier the D-G8-3 migration gate PASSED; D-G8-3A/B/D — **FIXED LOCALLY, NOT PUSHED**; D-G8-3C → **D-G8-4D**; earlier the same day D-G8-2A — **HOSTED ACCEPTED / CLOSED** on the `app_rw` Preview; closure digest `94491ea1…`; D-G8-2B deferred; D-G8-3A/B/C/D, D-G8-4A/B/C, D-G8-5, D-P1 OPEN / PRE-GATE-9; Gate 9 NOT started)

**2026-09-16 (latest) — D-P1: FIXED LOCALLY / NOT PUSHED. Code-only, no migration.**
...
- **A SECOND poisoning vector was found during implementation:** `allOpps` is itself narrowed by `scopeIds` (the ecosystem scope), so the *scope selector* corrupted the row too. That is why the fix is structural rather than "pass the unfiltered array".
...
- **Hosted untouched:** no push, no deploy, no hosted read or write, no env / `DATABASE_URL` / `DATABASE_URL_OWNER` / Production change. Hosted record stands: migrations 108 · business `c56a1d229e483f2b` · world `933a5e30d79297a4` · security `2a5ea0509145ee81` · 31/0. **Hosted `pipeline_snapshots` 2026-09-15 and 2026-09-16 were NOT rewritten** and no repair routine was added — they remain evidence pending a separate owner decision.
