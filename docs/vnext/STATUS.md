...
| **vNext isolated database** | **PREVIEW READY** (database only) — initialized, marked, seeded, reconciled; **Slice 2A schema + Globex plan installed 2026-09-14T16:49Z, with a known world defect** | target ref `mejokqxriwyawfhawuxu` | 2026-09-14T16:49Z | **The hosted world has no pursuit team** (0 `pursuit_team_requirements`, 0 `pursuit_team_members`, Globex ledger 9 not 10) — the in-place seed truncates the requirements a migration inserted and never replays it. Owner decision on the fix (`SESSION-HANDOFF.md` → exact next step), then the Vercel wiring (row below) | 103/103 migrations. Coordination verifier on hosted 103 pass / 2 fail / 11 not run (both failures = the team defect; 4 as-`app_rw` checks cannot run because `postgres` cannot `SET ROLE app_rw` here). | Migrated **102/102** from empty. `environment_identity` = `demo` / `is_synthetic=true` / "pursuitos-vnext — isolated synthetic preview". Seed 10/10 layers, `verify()` 17/17. Reconciled **exactly**: 3 orgs · 14 companies · 19 opportunities · 11 open · $8,040,000 · 14 pursuits; manifest digest `be0da833990ce436` = certified. 0 messages. Identity **proven distinct** from `qifatlqxfuhwrwvpbwsc`. Still unverified: whether it is a Supabase branch or a standalone project. One transient `28P01` on the first probe, then consistent success — `ENVIRONMENT-MAP.md` §10 |
...
| **Pursuit Coordination — Vertical Slice 2A** | P3 | **PREVIEW READY** (local synthetic) — hosted schema + data **installed**, hosted verification **PARTIAL**; **not** DEMO CERTIFIED | `roadmap/pursuitos-vnext` @ `c79efc6` | 2026-09-14T16:49Z | ~~Apply 0103 + run `demo-plan-story.ts` on `mejokqxriwyawfhawuxu`~~ **done 16:49Z**. Next: owner decision on the hosted no-team defect (Globex owner line differs from S2A-12 on hosted), then arm the flag on the Preview scope (owner-approved), then product review | Goal → Plan → Motion → Action on Pursuit Detail. Migration 0103 (3 tables, additive). Two INTERNAL_WRITE skills; no send path. Deferred: Today integration, goal editing, plan closure on WON/LOST (see `SESSION-HANDOFF.md`) |
