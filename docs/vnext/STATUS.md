...
**Last updated:** 2026-09-17 — **P2 PORTFOLIO PERTINENCE IMPLEMENTED LOCALLY / NOT PUSHED.** *"Why THIS pursuit rather than another?"* — five declared signals, deterministic, no model, **no persistence and no migration**. `portfolio-pertinence` **66/0**; SEEDED **1729/0** (1663 + the new suite, nothing else moved), FRESH **238/0**, EITHER **215/0**, unit **429/429**; `certify-world --runs 2` **98 clean / 0 failures**, digest **`f72d1ff0d6b07b42` unchanged**. The value architecture is the heart of it: **PIPELINE stage-weighted value and MODELED customer impact are never compared as dollars** — `value/case.ts` states modelled impact is *not our revenue* — so each is normalized only within its own basis cohort and only unitless relative standing crosses the boundary. **Hosted unchanged at migration 111** (business `6db00c6841c51fc3` · world `f2d83f71489c6682` · security `092a20af64a0444e` · 31/0); nothing pushed. **P45 hosted record fully preserved**; no runtime, governed-action or plan-surface file touched. *(Earlier entries below are a dated historical log; where they say an item is OPEN, that was true on that date and has since been superseded.)*

**2026-09-17 (latest) — P2 PORTFOLIO PERTINENCE: IMPLEMENTED LOCALLY / NOT PUSHED.**
