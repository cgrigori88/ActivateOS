...
**Last updated:** 2026-09-16 — **DEFECT P45-D1 CORRECTED LOCALLY / NOT PUSHED.** The hosted functional gate found that the runtime **could not execute under `app_rw`**: it appended its ledger event then UPDATEd the row, and `change_ledger` is append-only (`INSERT, SELECT`, zero column-level UPDATE). Fixed by writing the linkage **in the INSERT**; **no migration, no grant, no RLS change**. The local suite now executes through the real `app_rw` identity and passes **50/50**, with a negative control proving it would have caught the defect. **Real-runtime-identity testing is now a PERMANENT P45 acceptance requirement.** Hosted is untouched at migrations **109** · business `6abe424f43bff901` · world `c299c6e372c686c4` · security `569e5497a7622048` · 31/0; the hosted functional rerun is separately authorised. **P45-1 NOT CLOSED. Slice 2 NOT STARTED.** *(Earlier entries below are a dated historical log; where they say an item is OPEN, that was true on that date and has since been superseded.)*

**2026-09-16 (latest) — DEFECT P45-D1: CORRECTED LOCALLY / NOT PUSHED. Real-identity testing now permanent.**
