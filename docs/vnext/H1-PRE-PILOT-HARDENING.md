...
### D-G8-4 HOSTED ACCEPTANCE — 4A / 4B / 4D ACCEPTED · **4C NOT ACCEPTED (defect found)** (2026-09-16)

`982a01f` was pushed (fast-forward `c44cc89..982a01f`, no force/rewrite/tags) and deployed as
**`dpl_29srUo2LNtc6FW7dm12K3ZVxRGo5`** (`pursuitos-demo-l1v59r2o3-…`), Preview, branch
`roadmap/pursuitos-vnext`, **READY**, the branch alias target. No migration applied, no Vercel env,
`DATABASE_URL`, `DATABASE_URL_OWNER`, role, RLS, grant or policy change; Production and
`qifatlqxfuhwrwvpbwsc` untouched; sending off.

**`/api/build`:** `mejokqxriwyawfhawuxu` · `app_rw` · bypassRls false · tenantEnforcement true · probe
live · sending off. Env unchanged (18 variables).

**Database / security: PASS — D-G8-4 is code-only and the database proves it.** migrations **107** ·
business-data **`c56a1d229e483f2b`** · whole-world **`ff4a3f28c4940a9d`** · security
**`f31e51d50e9dec49`** · manifest `14e2e97f8453fb75` · protected **31 / 0 unsafe** · `app_rw` LOGIN true
/ BYPASSRLS false. Against the accepted D-G8-3 snapshot: **0 of 155 per-table fingerprints changed**,
and policies, RLS, grants, triggers, roles, `protectedFns`, `functions` and `applied` are all identical.
`pipeline_snapshots` stayed at 2 rows, so **CFR-1.1 did not apply**.

**Crawl: PASS.** Two signed-in 37-room crawls, 4 passes. **37/37 rooms 200 and all 4 passes
byte-identical.** No empty state, no missing authorized data, **"Meridian" appears 0 times**, joint-room
boundaries unchanged, owner-only paths healthy, palette healthy. **D-G5-1, D-G8-1, D-G8-2A and
D-G8-3A/B/D all remain closed**; the CDW list label is unchanged; the D-G8-1 stakeholder order is intact.

**Rendered differential — 3 rooms, every one explained.** Against the accepted D-G8-3 crawl, 34/37 rooms
are byte-identical and three differ, all from approved D-G8-4 corrections, with **line counts unchanged**
(1329→1329, 686→686, 139→139) so no row entered or left any surface:

1. `/pipeline` — **the pre-registered Stark disclosure.** `Contract expiry` → **`Contract expiry /
   Renewal`**, rendered as `Stark Industries LLC | conflicting | Contract expiry / Renewal | contradicted
   — 2026-10-31 vs 2026-12-12 | partner-reported vs customer declared`. The dates are the **union** of
   the competing dates, and the rendering is stable in all 4 passes.
2. `/partners/<partner>/review` — the same Stark disclosure on a second consumer.
3. `/pursuits/<globex>` — D-G8-4B's new categorical line: **`Most common outcome CLOSED_WON (4 of 4 ·
   100%)`**, beside the unchanged `4 won · 0 lost` counts and small-sample warning.

**D-G8-4A: ACCEPTED.** Deployed order is confidence → recency → `PROVENANCE_STRENGTH`, with
**CUSTOMER_DECLARED (0.95) > THIRD_PARTY_VERIFIED (0.85)**, **THIRD_PARTY_UNVERIFIED (0.45) > INFERRED
(0.30)** and **HUMAN_ASSERTED = SECOND_PARTY (0.70)** — deliberately unordered, so an equal-strength tie
with different output is UNRESOLVED. `projection.ts` and `state.ts` both carry the identical three-key
comparator. `LADDER_RANK` appears nowhere in the provenance module's code, and
`primaryLifecycleOutcome` is driven by `LIFECYCLE_STATE_RANK`, not provenance. On live hosted data the
Stark tie returns **UNRESOLVED** with both tied events sharing one lifecycle state.

**D-G8-4B: ACCEPTED.** The median is computed over the whole eligible terminal population. Independently
recomputed from raw hosted rows: **4 terminal outcomes, 0 timestamped → median `null` (UNKNOWN, never
zero)**, and mode **CLOSED_WON, 4 of 4, 100%** — exactly what the deployment renders. No categorical
value is labelled a median anywhere in the 37 rooms; the only other "median" strings are the
pre-existing numeric accept-latency statistics on `/insights` and partner detail, and **both rooms are
byte-identical to the accepted D-G8-3 crawl**.

**D-G8-4D: ACCEPTED.** Deployed precedence is explicit eligible seed → unique highest score →
`company_id` NULL, with no input-order, name, alphabetical, uuid, `created_at` or heap fallback in the
seed block. Verified on hosted inside a **rolled-back** transaction: an entirely unscored population
leaves `company_id` **NULL**, and the readers' LEFT JOIN still returns the campaign. **Zero residue** —
campaigns 0, populations 4, members 31, exactly as before. No new status enum, no `seed_company_id`.

**Tenant / consent: PASS.** Context-less `app_rw` sees 0 rows on all seven org-scoped tables probed;
the two already-accepted policy-visible tables are unchanged (`environment_identity` 1 row;
`pursuit_team_requirements` 5 rows, **0 org-owned**); Vertex sees exactly its 13 pursuits; the **Meridian
pursuit stays hidden**; joint 1 / active partnership 1 / context grants 2; settlement non-party **zero**;
**no cross-transaction `app.org_id` leak**. No new isolation failure.
...
absent · no external or webhook delivery.

---

#### D-G8-4C — NOT ACCEPTED. Defect found during hosted acceptance.

**`companies.normalized_name` does not hold normalized values on this world, so ladder rung 3 is dead.**
The column stores the raw legal name (`"Stark Industries LLC"`, `"Globex Manufacturing Inc."`) while
`normalizeCompanyName()` produces `"stark industries"`, `"globex manufacturing"`. Agreement across the
14 hosted companies is **0 / 14**, so rung 3 — *exact normalized canonical match* — **never fires**, and
identity silently falls through to rung 4, *unique fuzzy*.

**Demonstrated consequence.** Typing the **exact** legal name `"Initech Financial"` — which matches
exactly one company by name — returns **AMBIGUOUS (2 candidates)**, because rung 4's `ilike '%…%'` also
catches `"Initech Financial (expansion)"`:

| typed | result | exact-name matches | fuzzy matches |
|---|---|---|---|
| `Initech Financial` | **AMBIGUOUS (2)** | 1 | 2 |
| `Globex Manufacturing Inc.` | RESOLVED via **UNIQUE_FUZZY** | 1 | 1 |
| `Stark Industries LLC` | RESOLVED via **UNIQUE_FUZZY** | 1 | 1 |
| `Acme Robotics` | RESOLVED via **UNIQUE_FUZZY** | 1 | 1 |

**Severity: fail-safe, not wrong.** The ladder never resolves to the *wrong* company — it returns
unresolved instead — and ask-scope correctly blocks execution on that ambiguity. Nothing in the 37-room
crawl changed because no certified room performs a colliding exact-name lookup. But an exact legal name
**is** identity evidence, the approved ladder intends to honour it at rung 3, and on hosted it does not.
This is a defect in the D-G8-4C implementation, not in the ruling.

**The other D-G8-4C parts did pass** and are recorded for when 4C is re-submitted: canonical id resolves;
a unique fuzzy candidate resolves; a multi-candidate fuzzy match is **UNRESOLVED (7 candidates)**, never
the shortest or first; no shortest-name, alphabetical, uuid or row-order rule remains anywhere.
**ask-scope: PASS** — ambiguity **fails closed** (`allowed = false`) as its own `ambiguous_account`
outcome, never as out-of-scope, carrying a **count only** with no candidate name or id leaking; a
genuinely out-of-scope account is still refused *as* out-of-scope; an unambiguous in-scope account is
still allowed. **In-force facts: PASS** — the same explicit `asOf` is reproducible within one evaluation,
a historical `asOf` (2020-01-01) is honoured and reproducible, and the selector never reads the clock
itself; 3 hosted facts carry a validity window. **plan-loaders: PASS** — VERIFIED_DATE (1) outranks
INFERRED_WINDOW (2).

**DISPOSITION.**
- **D-G8-4A — HOSTED ACCEPTED / CLOSED**
- **D-G8-4B — HOSTED ACCEPTED / CLOSED**
- **D-G8-4D — HOSTED ACCEPTED / CLOSED**
- **D-G8-4C — NOT ACCEPTED, OPEN.** Deployed and fail-safe, but rung 3 is inert on hosted data. Needs an
  owner decision on the fix (normalize at comparison time, backfill `normalized_name`, or prefer an exact
  `legal_name` match ahead of fuzzy) before it can be closed.
- **D-G8-4 overall — NOT CLOSED**, pending 4C.
...
### D-G8-4C HOSTED ACCEPTED / CLOSED — **D-G8-4 OVERALL CLOSED** (2026-09-16)

`48a6157` was pushed (fast-forward `982a01f..48a6157`, no force/rewrite/tags) and deployed as
**`dpl_Ey7DdPDSWzsk5dJ3ASMbqbLmtxf7`** (`pursuitos-demo-b1y94zkqj-…`), Preview, branch
`roadmap/pursuitos-vnext`, **READY**, the branch alias target. Code-only: no migration created or
applied, no `companies.normalized_name` repair, no hosted business-data, Vercel env, `DATABASE_URL`,
`DATABASE_URL_OWNER`, role, RLS, grant or policy change; Production and `qifatlqxfuhwrwvpbwsc`
untouched; sending off.

**`/api/build`:** `mejokqxriwyawfhawuxu` · `app_rw` · bypassRls false · tenantEnforcement true · probe
live · sending off. Env unchanged (18 variables).

**Database / security: unchanged.** migrations **107** · business-data **`c56a1d229e483f2b`** ·
whole-world **`ff4a3f28c4940a9d`** · security **`f31e51d50e9dec49`** · manifest `14e2e97f8453fb75` ·
protected **31 / 0 unsafe** · `app_rw` LOGIN true / BYPASSRLS false. Settlement function posture intact
(10 columns, SECURITY DEFINER, STABLE, `pg_catalog, public, pg_temp`, ACL `{postgres,app_rw}`).

**Deployed resolver semantics (verified in the built commit).** Rung 4 applies
`normalizeCompanyName` to **both** the typed input and `legal_name`. The resolver reads
`companies.normalized_name` **zero** times; there is **no** `length(` rule, **no** `order by` anywhere in
the module, and no raw-`legal_name`-only rung was added. The ladder is intact:
**canonical id → ID-type alias → name/domain alias → normalized exact canonical match → unique fuzzy →
unresolved.** One match resolves, two or more are AMBIGUOUS, zero falls through to fuzzy.

**Initech regression on the REAL hosted data (read-only, `txid` null): FIXED.**

| check | result |
|---|---|
| hosted canonical names normalizing to `initech financial` | **exactly 1** → `Initech Financial` |
| fuzzy population (the defect conditions still present) | **2** → `Initech Financial`, `Initech Financial (expansion)` |
| `"Initech Financial"` | **RESOLVED via NORMALIZED_NAME** (was AMBIGUOUS) |
| the `(expansion)` sibling | does **not** make it ambiguous |
| stored `normalized_name` for that row | `"Initech Financial"` ≠ `"initech financial"` — the resolver did not need it |
| `"Globex Manufacturing Inc."`, `"Stark Industries LLC"` | **RESOLVED via NORMALIZED_NAME** |

**Normalized-exact ambiguity: PASS.** One candidate → RESOLVED. Hosted carries **no natural
duplicate-normalized pair (0)**, so two names normalizing alike (`Zephyr Analytics Inc` / `Zephyr
Analytics LLC`) were proven **AMBIGUOUS (2)** inside a **rolled-back** transaction — `companies` back to
**14** rows, **zero residue**. Zero normalized-exact candidates (`"Initech Fin"`) correctly proceeds to
fuzzy and stays **UNRESOLVED (2)**, so the shortest candidate did not win merely for being shortest.

**Fuzzy / alias: PASS.** A unique fuzzy candidate still resolves; a multi-candidate fuzzy match
(`"o"`, 7 candidates) remains UNRESOLVED. Hosted `company_aliases` is still **0**, so ID-type-above-
name/domain precedence, conflicting-ID-aliases-unresolved and explicit-namespace resolution rest on the
deployed code plus the local `semantic-determinism` suite (**50/0**). No durable aliases created.

**ask-scope: PASS.** Ambiguity **fails closed** (`allowed = false`) as its own `ambiguous_account`
outcome, never as out-of-scope, carrying a **count only** — asserted that no hosted company name or id
appears in the response. And the point of the correction: **the now-resolvable exact name
`"Initech Financial"` is ALLOWED**, where before it was blocked as ambiguous.

**Crawl: PASS, with NO new rendered difference.** Two signed-in 37-room crawls, 4 passes: **37/37 rooms
200, all 4 passes byte-identical**, and **37/37 byte-identical to the accepted D-G8-4 crawl** — exactly as
predicted, because no certified room performs the colliding exact-name lookup that exposed the bug. No
missing authorized data, **"Meridian" 0 occurrences**, owner paths healthy, palette healthy, joint-room
boundaries unchanged, **CDW list label unchanged**.

**Prior items regression-checked only, no redesign: D-G5-1, D-G8-1 (Dana → Mike → Priya → Sarah),
D-G8-2A, D-G8-3A/B/D all remain closed.** The accepted D-G8-4 differences persist and are stable in all
4 passes: the **Stark disclosure** `Contract expiry / Renewal` and the **`Most common outcome`** line.

**Tenant / consent: PASS.** Vertex sees exactly its 13 pursuits; the **Meridian pursuit stays hidden**;
joint 1 / active partnership 1 / context grants 2; settlement non-party zero; **no cross-transaction
`app.org_id` leak**. The only context-less visible tables remain the two already accepted —
`environment_identity` (1) and `pursuit_team_requirements` (5, **all `org_id IS NULL` templates, 0
org-owned**) — with policies and RLS byte-identical. **No new isolation failure.**

**Database / immutability: PASS — the deployment mutated nothing.** Against the accepted D-G8-4
snapshot: **0 of 155 per-table fingerprints changed**; policies, RLS, grants, triggers, roles,
`protectedFns`, `functions`, `applied`, CREATE-on-public all identical. **`companies` content hash
identical at 14 rows — no company row rewritten and `normalized_name` untouched.**
`pipeline_snapshots` stayed at 2 rows, so **CFR-1.1 did not apply**. Only `extra.appRwSessions` differs.
...
- **D-G8-4A — HOSTED ACCEPTED / CLOSED**
- **D-G8-4B — HOSTED ACCEPTED / CLOSED**
- **D-G8-4C — HOSTED ACCEPTED / CLOSED**
- **D-G8-4D — HOSTED ACCEPTED / CLOSED**
...
## D-P1 — FIXED LOCALLY (2026-09-16) · CODE-ONLY, NO MIGRATION · NOT PUSHED

**The defect, as confirmed at the design gate.** `/pipeline` computed its headline aggregates from the
**rendered** projection and then wrote them straight into the canonical daily row:

```
opps     = horizon ? allOpps.filter(o => closeDate <= horizon) : allOpps   // ?timeframe=7|30|90
open     = opps.filter(not closed)
total    = sum(open.amountUsd)
weighted = weightedPipelineValue(opps)
insert into pipeline_snapshots (org_id, taken_on, open_count, open_usd, weighted_usd, crm_usd)
values ($1, now()::date, open.length, total, weighted, tieOut?.crmUsd)
on conflict (org_id, taken_on) do update set ...
```

So **merely looking at `/pipeline?timeframe=7` overwrote today's canonical snapshot with 7-day totals**,
and the next unfiltered view silently overwrote it back. A pure read mutated committed history, and the
value persisted depended on which URL a human happened to open last. Trend, radar and any downstream
history read from that row.

### The defect class, widened at implementation and ACCEPTED by owner decision

`?timeframe=` was not the only way in. `allOpps` is itself narrowed by `scopeIds` (the ecosystem scope),
so the canonical row was corrupted by the **scope selector** as well. The owner has ruled that this is
**not unrelated scope creep** but a second instance of one defect class, which is D-P1's real subject:

> **FILTERED-VIEW SNAPSHOT POISONING** — any caller-side narrowing of the opportunity set reaching the
> canonical `pipeline_snapshots (org_id, taken_on)` row.

That is exactly why the accepted fix is structural rather than a "pass the unfiltered array instead"
patch: **the page has more than one filtered view of the same set, and a caller-discipline fix would have
to be re-proved at every call site forever.** The approved writer closes **timeframe filtering, ecosystem-
scope filtering, and any future caller-side filtered projection** in one move. **No separate workstream is
opened for the scope selector** — it is covered here.

### The fix — Owner Ruling 1, option (B)

`src/lib/pipeline/snapshot.ts` is a new canonical writer with a deliberately minimal surface:

```ts
export async function upsertCanonicalPipelineSnapshot(db: Db, orgId: string): Promise<void>
```

It accepts **no caller-computed value at all** — no `open_count`, `open_usd`, `weighted_usd`, `crm_usd`,
`timeframe`, horizon or filtered opportunity set — and derives every persisted field itself from the
org's **full, unfiltered** opportunity set, using the canonical stage curve via `loadStageWeights`
(partner override → org default → `STAGE_PROBABILITY`). **Poisoning is impossible by API shape, not by
caller discipline.** `/pipeline` now holds no snapshot write of its own; its one call is
`await upsertCanonicalPipelineSnapshot(db, tieOrgId);`.

**Owner Ruling 2 — read-triggered write KEPT.** History still accrues just by looking. No cron, no job,
no endpoint, no scheduler was added; the trigger point in `/pipeline` is unchanged. Only *what* is
persisted changed.

**Owner Ruling 3 — `crm_usd` is recomputed INSIDE the writer.** The page passes none. The writer runs the
same `distinct on (company_id, lower(opportunity_name)) … order by … reported_at desc, id desc` latest-per-
opportunity reduction over `crm_snapshots`, excluding closed stages and null amounts, and persists `null`
when there is nothing to report. **This required no architectural change** — the reduction is a plain
query against a table the writer already had the org id for — so the ruling's "STOP and report before
widening scope" clause was not reached.

### Certification — `dp1-snapshot-boundary`, 26 checks over the 21-item list, 26/26

New SEEDED / `SEEDED_CLONE` suite (`scripts/dp1-snapshot-boundary-verify.ts`), registered in
`verify-classes.ts`. **Every expected value is computed independently by the suite — the writer never
certifies itself.** The fixture is a disposable org whose opportunities close far beyond 90 days, so a
`?timeframe=` view is *materially* different (checks 3/4/5 assert that difference explicitly, so the
byte-identity result cannot pass vacuously).

| # | Assertion |
|---|---|
| 1 | Independent canonical metrics computed from the fixture (not from the writer) |
| 2 + 11 | `open_count` is canonical |
| 3 / 4 / 5 | `?timeframe=7 / 30 / 90` leaves the canonical snapshot **byte-identical** — and the filtered projection is materially different |
| 6 | The writer's public API accepts no filter/aggregate — arity is exactly `(db, orgId)`; no third parameter exists through which view state could reach the snapshot |
| 7 | Five repeated renders cause **zero** snapshot content movement |
| 8 | Returning to the unfiltered view repairs nothing, **because nothing was damaged** |
| 9 | **Negative control:** the PRE-FIX flow, replicated literally, **does** poison the canonical row with 7-day totals — and the fixed writer restores canonical values and cannot reproduce that mutation |
| 10 | Six concurrent requests cannot leave a filtered value persisted — all writers agree |
| 12 | `open_usd` is canonical |
| 13 | `weighted_usd` is canonical (against the canonical `STAGE_PROBABILITY` curve) |
| 14 | `crm_usd` is canonical and derived **inside** the writer — the page passes none |
| 15 | Org A cannot modify org B's snapshot under `app_rw`; org B's row is unchanged |
| 16 | `app_rw` sees exactly its own org's snapshot under tenant context |
| 17 | Prior-date rows remain byte-identical through repeated writes |
| 18 | Today's row still equals the **independent** canonical recomputation |
| 19 | **CFR-1.1** — at most one row per org/date, and today's equals an unfiltered canonical recomputation |
| 20 | **Static call-site guard:** `/pipeline` holds no `insert into` / `update pipeline_snapshots` of its own, has exactly one call site, and passes exactly two arguments, none of which is a rendered aggregate or view-state identifier |
| 21 | Send activity — `0 / 0 / 0 / 0 / 0` and unchanged |

Check 20 was added after the first pass because arity alone cannot prove the *page* stopped computing the
values; its regexes were negative-controlled and correctly reject `(db, orgId, total)`, `(db, orgId, weighted)`,
a second call site and an inline `insert into pipeline_snapshots`.

**One first-failure captured and recorded, not overwritten.** The first suite run was **24/25** —
`13: weighted_usd is canonical — 447500.00 vs 462500`. The failure was captured before any re-run.
Diagnosis: **the test was wrong, not the writer.** The suite's hardcoded default curve used
`qualification 0.25` / `negotiation 0.8`, but the canonical `STAGE_PROBABILITY`
(`src/lib/opportunities/lifecycle.ts`) is `qualification 0.2` / `negotiation 0.75`. The correct arithmetic
— `400000×0.6 + 250000×0.75 + 100000×0.1 + 50000×0.2 = 447500` — is exactly what the writer produced.
The suite now reads the canonical constant instead of guessing it, which keeps the expectation independent
of the writer while making it agree with the product's documented curve.

### Full local certification

`tsc` clean · `npm test` **429/429** · `npm run build` clean · **`dp1-snapshot-boundary` 26/0** ·
`dg85-determinism` 19/0 · `persisted-determinism` 17/0 · `semantic-determinism` 50/0 ·
`partnership-app-rw` 117/0 · `tenant-isolation` 205/0 · `search-path` 39/0 ·
catalogue guard **31 protected / 0 unsafe** · `app-rw-rehearsal` **38/38 rooms + 6/6 consent** ·
**`certify-world --runs 2`: 90 clean / 0 failures, CERTIFICATION INTEGRITY PASS, digest
`e98b43254f98d5ec` identical at start, after run 1 and after run 2.**

**Canonical local world unchanged.** `world-fingerprint` **`e98b43254f98d5ec` · 154 tables · 1051 rows** —
the pre-registered value. Local `pipeline_snapshots` still holds its single canonical row
(`2026-09-14 · 11 · 8040000.00 · 3361500.00 · crm null`), **untouched**; every fixture write in this work
happened inside disposable seeded clones, and the catalogue probe committed nothing
(`txid_current_if_assigned()` null). Send counters `0 / 0 / 0 / 0 / 0`.

**No migration.** `supabase/migrations` holds **108** files, highest `0108_dg85_shared_in_evidence_determinism.sql`
— nothing was added. No schema, RLS, grant, policy, role or function change; the local security-catalogue
hash is steady at `698c56c7abc41908`. *(The local seeded world has no `schema_migrations` ledger table —
that is the known 154-vs-155 table difference against hosted, not a new condition.)*

**Hosted untouched, exactly as instructed.** No push, no deploy, no hosted read or write, no Vercel env
change, no `DATABASE_URL` / `DATABASE_URL_OWNER` change, no Production, no `qifatlqxfuhwrwvpbwsc` contact.
The hosted record of record therefore stands unchanged: migrations **108** · business-data
**`c56a1d229e483f2b`** · whole-world **`933a5e30d79297a4`** · security **`2a5ea0509145ee81`** ·
protected **31 / 0**. Hosted `pipeline_snapshots` was **not** modified, **not** rewritten, and no repair
routine was added.

**Hosted snapshot state — evidence-accurate statement.** The last read-only hosted verification during the D-P1 design gate found the 2026-09-15 and 2026-09-16 rows **equal to independent canonical recomputation** (`open_count` 11, `open_usd` 8,040,000, `weighted_usd` 3,361,500) and concluded **no hosted poisoning had occurred**. The local implementation made **no hosted contact**, so it can neither confirm nor disturb that finding. Their current state will be **re-verified read-only as the first step of hosted D-P1 acceptance**. No claim is made here that those
rows are poisoned or wrong, and none is made that they are still clean: this work has no evidence either way.

**Not in scope, not done:** no Pipeline UX redesign, no UI cleanup, no D-G8 reopening, no Gate 9 work.

**DISPOSITION: D-P1 — FIXED LOCALLY, NOT PUSHED.** Hosted acceptance is a separate gate requiring its own
approval, because a push auto-deploys the branch Preview.


---

## D-P1 — HOSTED ACCEPTED / CLOSED (2026-09-16) · PRE-GATE-9 HARDENING COMPLETE

**Deployment.** `f936fbe` (carrying `6f3e65d`) pushed fast-forward `ae424d5..f936fbe`, deployed as
**`dpl_FnUKb4vWHMQkpAKAbxtDxCKeyyYp`** — project `pursuitos-demo`, Preview, branch
`roadmap/pursuitos-vnext`, READY. `/api/build`: projectRef `mejokqxriwyawfhawuxu` · role **`app_rw`** ·
bypassRls **false** · tenantEnforcement **true** · probe **live** · externalSendingArmed **false**.
Vercel env **byte-identical, 38 entries** pre→post; `DATABASE_URL` still `app_rw`, `DATABASE_URL_OWNER`
still the owner. No migration — level stays **108**.

### Part 0 — PRE-DEPLOY HOSTED SNAPSHOTS: CLEAN (before any push)

Read-only, before pushing anything. `pipeline_snapshots` holds **2 rows, both Vertex Systems**. Both
were checked against an **independent pure-SQL recomputation** that never reads `pipeline_snapshots`:

| date | stored | independent canonical | verdict |
|---|---|---|---|
| 2026-09-15 | 11 · 8040000.00 · 3361500.00 · null | 11 · 8040000 · 3361500 · null | **EQUAL** |
| 2026-09-16 | 11 · 8040000.00 · 3361500.00 · null | 11 · 8040000 · 3361500 · null | **EQUAL** |

`txid_current_if_assigned()` NULL — nothing written. **PRE-DEPLOY HOSTED SNAPSHOTS — CLEAN. No repair
required, and none was performed.** This settles the question the pre-push evidence correction left
open: the rows were canonical at the moment of deployment.

### Deployed code identity

Verified against the exact commit `/api/build` reports. `src/lib/pipeline/snapshot.ts` present; writer is
`upsertCanonicalPipelineSnapshot(db: Db, orgId: string)`; its body references no caller timeframe,
horizon, scope or aggregate; it derives `open_count`, `open_usd`, `weighted_usd` and `crm_usd` itself from
org-wide sources, with `taken_on` from the database's `now()::date`. `page.tsx` holds **no**
`insert into` / `update pipeline_snapshots`, has **exactly one** call site passing only `(db, tieOrgId)`,
**inside `withTenant`** — no owner connection anywhere in the pipeline path — and retains both snapshot
reads, the timeframe rendering semantics and the ecosystem-scope rendering semantics.

### Parts E–L — the filtered-view proof on real hosted data

Canonical baseline after an unfiltered render:
`{"open_count":11,"open_usd":"8040000.00","weighted_usd":"3361500.00","crm_usd":null}` —
**`CANONICAL_BASELINE_AFTER_UNFILTERED` = `9b2c3c6ce81d9f12`**, equal to independent recomputation.

Every filtered view was checked twice over: the rendered projection must be **correct for that filter**
(against an independent SQL recomputation of the same horizon/scope), and the canonical row must be
**byte-identical** to the baseline.

| part | view | rendered | independently expected | canonical row |
|---|---|---|---|---|
| F | `?timeframe=7` | 0 · $0 · $0 | 0 · $0 · $0 | `9b2c3c6ce81d9f12` unmoved |
| G | `?timeframe=30` | 2 · $800K · $160K | 2 · $800K · $160K | `9b2c3c6ce81d9f12` unmoved |
| H | `?timeframe=90` | 11 · $8.04M · $3.36M | 11 · $8.04M · $3.36M | `9b2c3c6ce81d9f12` unmoved |
| I | `?scope=PARTNER:WWT` | 7 · $5.66M · $2.14M | 7 · $5.66M · $2.14M | `9b2c3c6ce81d9f12` unmoved |
| J | scope **+** `timeframe=7` | 0 · $0 · $0 | 0 · $0 · $0 | `9b2c3c6ce81d9f12` unmoved |

The ecosystem scope used is the application's **real** `?scope=` mechanism (re-authorized server-side),
narrowing 10 companies / 19 opportunities to 6 / 11 — **materially** different, and the original accepted
scope was restored afterwards with the rendered projection returning to 11 · $8.04M · $3.36M.
**Under the pre-fix code each of these would have persisted its filtered totals over canonical history.**

**Part K — repeated reads:** 8 renders across `7 / 30 / 90 / narrow scope / scope+30 / unfiltered`
produced **exactly ONE distinct canonical content hash**, `9b2c3c6ce81d9f12`. Zero content movement.

**Part L — concurrency:** 8 concurrent mixed requests across two authenticated sessions, all 200. After
they settled, today's row **exactly equalled independent canonical recomputation** and was still
byte-identical to the baseline. Last-write-wins is now harmless because every writer computes the same
canonical state. No locks were introduced. Prior-date rows byte-identical throughout every part.

### Part M — CFR-1.1: HOLDS, and is STRENGTHENED

At most one row per org/date (0 duplicate groups) · every prior-date row byte-identical to the Part 0
pre-test baseline · today's row equals independent unfiltered canonical recomputation · no unrelated
table changed (0 of 155). **D-P1 strengthens CFR-1.1**: canonical recomputation is now enforced by the
**writer boundary** rather than depending on the page caller getting it right. CFR-1.1 itself is
unchanged — not weakened, not rewritten.

### Part N — tenant / app_rw

`pipeline_snapshots`: **RLS true, FORCE RLS true**; policies `pipeline_snapshots_rw` (`{app_rw}`, ALL)
and `pipeline_snapshots_select` (`{authenticated}`, SELECT), both `is_org_member(org_id)` — the
org-scoped form resolving `app_current_org()`, never the catalogue `true` form. **All 380 policies are
byte-identical to the accepted D-G8-5 baseline — zero drift.** Connected as real `app_rw`
(BYPASSRLS false): no tenant context → **0 rows**; Org A context → exactly Org A's 2 rows; and under Org
A context, **insert for Org B refused by RLS**, **update 0 rows**, **delete 0 rows** — with a positive
control proving Org A *can* reach its own row, so those are real refusals and not a dead path. Every
write attempt ran inside a transaction that was **rolled back**; `pipeline_snapshots` returned to exactly
its 2 Vertex rows, hash `1a26527276d73039`. **Zero residue.** No owner-path shortcut exists.

### Parts O/P — certification and the 37-room crawl

`dp1-snapshot-boundary` **26/0** · `dg85-determinism` **19/0** · `persisted-determinism` **17/0** ·
`semantic-determinism` **50/0** · `partnership-app-rw` **117/0** · `tenant-isolation` **205/0** ·
`search-path` **39/0** · `app-rw-rehearsal` **38/38 rooms + 6/6 consent**.

**Crawl: 37/37 rooms 200, all 4 passes byte-identical, and 37/37 BYTE-IDENTICAL to the accepted D-G8-5
crawl — ZERO rendered difference**, which is exactly right for a persistence-only change. Meridian **0
occurrences** under Vertex; owner-only rooms (`/admin`, `/ops`, `/trust`, `/provider-health`) all 200;
palette healthy; joint boundaries unchanged; CDW label unchanged; the accepted Stark disclosure and the
most-common-outcome line both stable; **D-G8-1 order Dana → Mike → Priya → Sarah** intact. The
`/pipeline` headline read 11 · $8.04M · $3.36M in all four passes. **D-G5-1, D-G8-1, D-G8-2A, D-G8-3,
D-G8-4 and D-G8-5 all remain closed.**

### Part Q — post-acceptance fingerprint

migrations **108** (latest `0108`, nothing pending, nothing applied-not-in-repo) · business-data
**`c56a1d229e483f2b`** · whole-world **`933a5e30d79297a4`** · security **`2a5ea0509145ee81`** · manifest
**`14e2e97f8453fb75`** · protected **31 / 0** — every one **UNCHANGED**.

> **0 of 155 per-table fingerprints moved.** `pipeline_snapshots` sat at
> `2:cb3b239393b46d6e266f2ae860905804` **before and after** — the repeated canonical upserts wrote
> byte-identical content, so the content fingerprint did not move at all. No UTC date rollover occurred
> during acceptance, so no new-date row was created and CFR-1.1's new-row clause was not reached.

Functions, triggers, policies, RLS, table grants, column grants, protected functions, schema-CREATE,
role membership and all 32 roles: identical pre→post.

### Part R — send / env / production safety

messages / action_outbox / email_events / sending_identities / sent_touches = **0 / 0 / 0 / 0 / 0** ·
`externalSendingArmed` **false** · `OUTREACH_AUTOSEND` and `RESEND_API_KEY` **absent** · no external or
webhook delivery. Vercel env **byte-identical (38 entries)**. Environment identity remains the isolated
synthetic preview (`demo`, `is_synthetic` true). **Production untouched; `qifatlqxfuhwrwvpbwsc` never
contacted.**

### Three first-failures, captured before any re-run — all TOOLING, none product

Recorded rather than quietly re-run, per the gate.

1. **`H2: the rendered 90-day projection differs from unfiltered` — FAILED.** Rendered
   11 / $8.04M / $3.36M, identical to unfiltered. **My assertion was wrong.** Read-only diagnosis showed
   all 11 open opportunities close **18–54 days** out with **no** null close dates, so the 90-day horizon
   is legitimately a no-op on this data (7-day → 0, 30-day → 2, 90-day → 11). The gate's actual
   requirement — that the page display the *correct* 90-day projection — was then verified against an
   independent SQL recomputation and **passed**. `H3`, the D-P1 assertion itself, passed on the first run.
2. **`N2: policy scope is the org-scoped tenant form` — FAILED.** My check looked for a literal
   `app.org_id` in the policy text; the scoping is expressed through `is_org_member(org_id)`, which
   resolves `app_current_org()`. **Expectation wrong, no drift** — proven by all 380 policies being
   byte-identical to the accepted baseline, plus the behavioural isolation results.
3. **An env "CHANGED DATABASE_URL" alarm — FALSE.** I diffed against a stale `vercel-env-closeout`
   snapshot predating the approved Gate 5 / Gate 8-Phase-2 role switches. Against the correct most-recent
   baseline the env is **byte-identical**.

Two harness defects were also fixed mid-run without affecting any product assertion: an RLS refusal
aborting the enclosing transaction and blocking later probes (fixed with savepoints, plus a positive
control), and two wrong JSON key names in an ad-hoc assertion script that made one check vacuous
(`0 of 0 tables`) and another read the wrong field — both corrected and re-run to the real result
(`0 of 155`).

`gate2.ts assert-same` additionally reported 5 items against its hardcoded **Gate 1b.1** constants
(migrations 105, business `79321d9130d1dc94`, world `de05e204801988d1`, security `30772757ebd4688c`).
Those are a superseded static baseline, not drift — migrations legitimately advanced 105 → 108 through
the approved D-G8-3 and D-G8-5 gates. Every **live** pre→post comparison in that same run passed.

### DISPOSITION

Every Part S condition is satisfied: canonical writer deployed · caller cannot pass filtered aggregates ·
`crm_usd` owned by the writer · timeframe 7, 30 and 90 cannot move the canonical snapshot ·
ecosystem-scope narrowing cannot move it · the combined filter cannot poison it · repeated filtered reads
cause zero content movement · concurrent filtered/unfiltered reads cannot leave filtered state ·
prior-date rows unchanged · today's row equals independent canonical recomputation · tenant isolation
intact · CFR-1.1 holds and is stronger · full certification clean · no unexplained hosted movement.

**D-P1 — HOSTED ACCEPTED / CLOSED.** Not to be reopened absent a new concrete defect.

**PRE-GATE-9 HARDENING — COMPLETE.** D-G5-1, D-G8-1, D-G8-2A, D-G8-3, D-G8-4, D-G8-5 and D-P1 are all
hosted-accepted and closed. D-G8-2B remains the recorded display-only backlog, deliberately deferred.
No further generic hardening sweep is to be started.

**HOSTED RECORD OF RECORD, unchanged:** migrations **108** · business-data **`c56a1d229e483f2b`** ·
whole-world **`933a5e30d79297a4`** · security **`2a5ea0509145ee81`** · protected **31 / 0** · `app_rw`
